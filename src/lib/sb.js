...
export const sb = {
  ...
  async del(table,id) { const r=await fetch(`${SB_URL}/rest/v1/${table}?id=eq.${id}`,{method:"DELETE",headers:sbHeaders}); if(!r.ok&&r.status!==404){console.error(`DB del ${table} FAILED [${r.status}]:`,await r.text().catch(()=>""));} return r.ok||r.status===404; },
  async delWhere(table,col,val) { const r=await fetch(`${SB_URL}/rest/v1/${table}?${col}=eq.${val}`,{method:"DELETE",headers:sbHeaders}); if(!r.ok&&r.status!==404){console.error(`DB delWhere ${table} FAILED [${r.status}]:`,await r.text().catch(()=>""));} return r.ok||r.status===404; },
}
