...
export function buildFullPrompt({ question, data, faqItems, role = 'master', extraContext = '' }) {
  ...
  parts.push(
    '[답변 지침]\n' +
    '1) 🚨 왁싱 관련 모든 질문(시술·사후관리·트러블·붉은기·인그로운·각질·피부 벗겨짐·통증·가려움·여드름·임산부·주기·효과·위생·남성고객·매장편의·클레임 응대 등)은 반드시 위 [하우스왁싱 업로드 문서] + [하우스왁싱 FAQ]에 근거해서만 답변하세요. 일반 지식·웹검색·추측·자체 추론 절대 금지.\n' +
    '2) 우선순위: 업로드된 문서 > FAQ > 매장 데이터. 업로드 문서에 답이 있으면 그것을 1순위로.\n' +
    '3) 매장 정책·가격·운영시간·예약/매출/고객 데이터도 위 컨텍스트에 근거해서만 답하세요.\n' +
    '4) 어디에도 답이 없으면: "하우스왁싱 FAQ/문서에 등록된 답변이 없어 정확히 안내드리기 어렵습니다. 대표에게 확인 후 답변드리겠습니다." 라고 답하세요. 일반 지식으로 채우지 마세요.\n' +
    '5) 답변 시 가능하면 출처(문서명·FAQ)를 짧게 인용 (예: "○○매뉴얼에 따르면...", "FAQ상 ~합니다").\n' +
