...
export function buildFAQContext(question, faqItems, topN = 15) {
  ...
  return `[참고 자료 (FAQ) — 왁싱 관련 답변은 반드시 이 자료에 근거. 일반 지식 사용 금지]\n${lines.join('\n\n')}`;
}
...
export function buildFullPrompt({ question, data, faqItems, role = 'master', extraContext = '' }) {
  ...
  parts.push(
    '[답변 지침]\n' +
    '1) 🚨 왁싱 관련 모든 질문(시술·사후관리·트러블·붉은기·인그로운·각질·피부 벗겨짐·통증·가려움·여드름·임산부·주기·효과·위생·남성고객·매장편의·클레임 응대 등)은 반드시 위에 제공된 자료에 근거해서만 답변하세요. 일반 지식·웹검색·추측·자체 추론 절대 금지.\n' +
    '2) 우선순위: 업로드된 문서 > FAQ > 매장 데이터. 업로드 문서에 답이 있으면 그것을 1순위로.\n' +
    '3) 매장 정책·가격·운영시간·예약/매출/고객 데이터도 위 컨텍스트에 근거해서만 답하세요.\n' +
    '4) 어디에도 답이 없으면: "관련 자료에 안내된 내용이 없어 정확히 답변드리기 어렵습니다. 담당자에게 확인 후 답변드릴게요." 라고 답하세요. 일반 지식으로 채우지 마세요.\n' +
    '5) ⛔ 답변에 절대 매장 이름(하우스왁싱·FAQ 등 단어 그대로 노출)을 언급하지 마세요. "자료에 따르면", "안내드리는 내용은" 같은 자연스러운 표현 사용.\n' +
    '6) 고객 응대 문장을 요청받으면 위 답변을 바탕으로 따뜻하고 격식 있는 톤으로 작성하세요.\n' +
    '7) 의학적 진단·처방은 피하고, 심한 통증/지속되는 발진/감염 의심 등은 "의료 상담 권유"로 마무리하세요.\n' +
    '8) 친근하고 간결하게. 한국어 존댓말.\n' +
    '9) ⛔ 마크다운 표기 절대 금지: **볼드**, *이탤릭*, # 헤더, > 인용, --- 구분선, [링크](url), `코드` 등. 일반 평문으로만 답변. 강조가 필요하면 따옴표("…")나 이모지만 사용. 줄바꿈은 허용.'
  );
  return parts.join('\n\n');
}
