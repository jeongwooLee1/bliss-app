...
const blockDurMin = (b) => {
  ...
  if (t && et) {
    ...
    fromEnd = Math.max(0, (eh*60+em) - (sh*60+sm));
  }
  const fromDur = Number(b.dur || 0);
  // dur과 endTime 차이가 다르면 더 긴 쪽 사용 — 시술시간(dur) 누락된 케이스 + endTime이 dur 미반영된 케이스 둘 다 보호
  const r = Math.max(fromEnd, fromDur);
  return r > 0 ? r : 30;
};
...
function Timeline({ data: _liveData, setData: _liveSetData, userBranches, viewBranches=[], isMaster, currentUser, setPage, bizId, onMenuClick, bizName, pendingOpenRes, setPendingOpenRes, naverColShow={}, scraperStatus=null, setPendingChat, setPendingOpenCust, unreadMsgCount=0, unreadDelayedCount=0, unreadSample=[], messagesPanelOpen=false, aiActiveCount=0, aiActiveSample=null, dismissAiActive, previewBlockStyle=false, betaGroupMode=false }) {
  ...
  const newCustClickIdx = useRef(0);
  const [dismissedNewCust, setDismissedNewCust] = useState(() => {
    try { return new Set(JSON.parse(localStorage.getItem('bliss_dismissed_newcust')||'[]')); } catch { return new Set(); }
  });
    ...
    Promise.all(batches.map(batch =>
      ...
      results.flat().forEach(p => {
        ...
        if (isDadam || isBarf) {
          ...
          if (bal > 0) {
            // trailing 충전금액 제거: "다담권 100만"·"바프권 30만"·"다담권 1,000,000원" → "다담권"/"바프권"
            const baseName = sn.replace(/\(잔액:[^)]*\)/,"").replace(/\s+[\d][\d,]*(\.\d+)?\s*(만원?|천|원)?\s*$/, "").trim();
            map[p.customer_id].push({ name: baseName || sn.replace(/\(잔액:[^)]*\)/,"").trim(), remain: bal, isDadam: true });
          }
  ...
  const [startHour, setStartHourRaw] = useState(() => tlDef("sh", 10));
  const [endHour, setEndHourRaw] = useState(() => tlDef("eh", 23));
  const [rowH, setRowHRaw] = useState(() => tlDef("rh", 10));
  const [colW, setColWRaw] = useState(() => tlDef("cw", 100));
  const [timeUnit, setTimeUnitRaw] = useState(() => tlDef("tu", 5));
  const [blockFs, setBlockFsRaw] = useState(() => tlDef("fs", 12));
  const [blockOp, setBlockOpRaw] = useState(() => tlDef("op", 80));
  ...
  // 지점 네비게이션 "정지 지점" 계산 + 단계 이동 (키보드·휠·터치 공통)
  const navLockRef = useRef(0);
  const _computeNavStops = (sr) => {
    // 각 지점 시작 위치 + (지점이 화면보다 넓으면) 그 지점 끝까지 보는 위치
    const cols = Array.from(sr.querySelectorAll(".tl-room-col"));
    if (!cols.length) return [];
    const offs = []; let lastBid = null;
    for (const c of cols) {
      const bid = c.getAttribute("data-branch-id");
      if (bid !== lastBid) { offs.push(c.offsetLeft); lastBid = bid; }
    }
    const contentW = sr.scrollWidth, clientW = sr.clientWidth;
    const maxScroll = Math.max(0, contentW - clientW);
    const viewCol = clientW - timeLabelsW;     // 시간축 제외 보이는 컬럼 폭
    const stops = [];
    for (let i = 0; i < offs.length; i++) {
      const S = Math.max(0, Math.min(maxScroll, offs[i] - timeLabelsW)); // 지점 시작 정렬
      stops.push(S);
      const rightEdge = (i + 1 < offs.length) ? offs[i + 1] : contentW;  // 이 지점 우측 경계
      if (rightEdge - offs[i] > viewCol + 4) {  // 화면보다 넓은 지점 → 끝까지 보는 중간 정지점 추가
        const E = Math.max(0, Math.min(maxScroll, rightEdge - clientW));
        if (E > S + 4) stops.push(E);
      }
    }
    return [...new Set(stops.map(x => Math.round(x)))].sort((a, b) => a - b);
  };
  const _navStep = (sr, forward) => {
    const stops = _computeNavStops(sr);
    if (!stops.length) return;
    const cur = sr.scrollLeft, eps = 4;
    let dest;
    if (forward) { dest = stops.find(s => s > cur + eps); if (dest == null) dest = stops[stops.length - 1]; }
    else { const p = stops.filter(s => s < cur - eps); dest = p.length ? p[p.length - 1] : 0; }
    navLockRef.current = Date.now() + 550;     // 단계 이동 직후 자석 스냅이 끼어들지 않게
    sr.scrollTo({ left: dest, behavior: "smooth" });
  };

  // 키보드 화살표 네비게이션: ←/→ 지점 단위 이동(넓은 지점은 끝까지), ↑/↓ 한 화면씩(끝난 다음부터)
  useEffect(() => {
    const onKey = (e) => {
      ...
      const sr = scrollRef.current;
      if (!sr || sr.offsetParent === null) return;       // 숨김(다른 페이지)이면 무시
      if (dragBlock || resizeBlock) return;              // 블록 드래그/리사이즈 중엔 무시
      e.preventDefault();
      if (e.key === "ArrowUp" || e.key === "ArrowDown") {
        const page = Math.max(120, sr.clientHeight - headerH - 6); // 헤더 가림 보정, 화면 끝난 다음부터
        sr.scrollBy({ top: e.key === "ArrowDown" ? page : -page, behavior: "smooth" });
        return;
      }
      _navStep(sr, e.key === "ArrowRight");  // ←/→ 지점(넓으면 끝까지) 단계 이동
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  ...
  useEffect(() => {
    ...
    let cooldown = false;
    const onWheel = (e) => {
      if (dragBlock || resizeBlock) return;
      const dx = e.shiftKey ? e.deltaY : e.deltaX;   // shift+휠 = 가로
      // 세로 의도가 더 크면(그리고 shift 아님) 네이티브 세로 스크롤 그대로
      if (!e.shiftKey && Math.abs(dx) <= Math.abs(e.deltaY)) return;
      e.preventDefault();
      if (cooldown || Math.abs(dx) < 6) return;
      cooldown = true;
      setTimeout(() => { cooldown = false; }, 380);   // 한 제스처당 한 지점 (관성 다중 이동 방지)
      _navStep(sr, dx > 0);
    };
    sr.addEventListener("wheel", onWheel, { passive: false });
      ...
      if (isDragging.current && dragSnapRef.current) {
        ...
        if (snap.time !== orig.time || snap.roomId !== orig.roomId) {
          ...
          if (isCopyDragRef.current) {
            ...
            // 결제 관련 시스템 태그("예약금완료") 제거 — 결제는 원본에만 귀속, 동반자엔 안 따라감
            const _prepaidTagId = (data?.serviceTags || []).find(t => t.name === "예약금완료")?.id;
            const _filteredTags = Array.isArray(block.selectedTags) && _prepaidTagId
              ? block.selectedTags.filter(t => t !== _prepaidTagId)
              : block.selectedTags;
            const newRes = {
              ...block,
              id: newId,
              ...
              isNewCust: false,
              selectedTags: _filteredTags,
              // 결제 정보 끊기 — 선결제·네이버결제는 원본에만 귀속 (동반자가 가져가면 "예약금완료" 자동 부착됨)
              isPrepaid: false,
              externalPrepaid: 0,
              externalPlatform: "",
              totalPrice: 0,
  ...
  const changeDate = (off) => {
    const d = new Date(selDate); d.setDate(d.getDate()+off);
    // 직원(staff)은 오늘 ±3일만
    if (currentUser?.role === 'staff') {
      const t0 = new Date(); t0.setHours(0,0,0,0);
      const lo = new Date(t0); lo.setDate(lo.getDate()-3);
      const hi = new Date(t0); hi.setDate(hi.getDate()+3);
      if (d < lo || d > hi) return;
    }
    setSelDate(fmtLocal(d));
  };
    ...
    <div style={{display:"flex",flexDirection:"column",flex:1,minHeight:0}}>
      ...
      {aiActiveCount > 0 && !messagesPanelOpen && (
        <div className="tl-alert-banner" style={{background:"#EDE9FE",borderBottom:"1px solid #C4B5FD",padding:"6px 12px",display:"flex",alignItems:"center",gap:T.sp.sm,flexShrink:0,cursor:"pointer",animation:"pendingBlink 2s infinite",width:"100%",boxSizing:"border-box"}}
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <span style={{fontSize:T.fs.xxs,color:"#6D28D9",marginLeft:8}}>직원이 답하지 않아 AI가 응대 중이에요. 클릭하면 그 대화로 이동, 확인했으면 [확인]을 눌러주세요.</span>
          </div>
          <span onClick={(e)=>{ e.stopPropagation(); if(aiActiveSample&&setPendingChat) setPendingChat(aiActiveSample); setPage&&setPage("messages"); }}
            style={{fontSize:T.fs.xxs,color:"#6D28D9",fontWeight:T.fw.bold,flexShrink:0,cursor:"pointer"}}>대화 열기 <I name="chevR" size={11} color="#6D28D9"/></span>
        ...
        return <div className="tl-alert-banner" style={{background:"#E0F2FE",borderBottom:"1px solid #7DD3FC",padding:"6px 12px",display:"flex",alignItems:"center",gap:T.sp.sm,flexShrink:0,cursor:"pointer",width:"100%",boxSizing:"border-box"}}
          onClick={()=>{
            ...
            if (first && first.user_id && setPendingChat) {
              setPendingChat({ user_id: first.user_id, channel: first.channel, account_id: first.account_id });
            }
      ...
      {(() => {
        ...
        return <div className="tl-alert-banner" style={{background:T.orangeLt,borderBottom:"1px solid #FFB74D",padding:"6px 12px",display:"flex",alignItems:"center",gap:T.sp.sm,flexShrink:0,cursor:"pointer",animation:"pendingBlink 2s infinite",width:"100%",boxSizing:"border-box"}}
          ...
          <div style={{flex:1,minWidth:0}}>
            <span style={{fontSize:T.fs.sm,fontWeight:T.fw.bolder,color:T.orange}}>{(() => {
              const ai = pendingList.filter(r => r.status === "request").length;
              const nv = pendingList.filter(r => r.status === "pending").length;
              if (ai > 0 && nv > 0) return `AI ${ai} · 확정대기 ${nv}`;
              if (ai > 0) return `AI 예약신청 ${ai}건`;
              return `확정대기 ${nv}건`;
            })()}</span>
      ...
      {/* New Customer Alert — 네이버 자동확정 신규고객 (확정 액션 불필요, 미리 알림용) */}
      {(() => {
        const allowedBranches = isMaster ? branchesToShow : allBranchList.filter(b => userBranches.includes(b.id));
        const newCustList = (data?.reservations||[]).filter(r =>
        ...
        return <div className="tl-alert-banner" style={{background:"#F3E8FF",borderBottom:"1px solid #C4B5FD",padding:"6px 12px",display:"flex",alignItems:"center",gap:T.sp.sm,flexShrink:0,cursor:"pointer",width:"100%",boxSizing:"border-box"}}
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <span style={{fontSize:T.fs.xxs,color:"#7C3AED",marginLeft:8}}>
              {newCustList.slice(0,3).map(r => {
                const br = allBranchList.find(b=>b.id===r.bid);
                return `${br?.short||br?.name||""} ${r.custName||"네이버"} ${r.date.slice(5)} ${r.time||""}`;
              }).join(" · ")}
              {newCustList.length > 3 ? ` 외 ${newCustList.length-3}건` : ""}
            </span>
          </div>
          <span style={{fontSize:T.fs.xxs,color:"#7C3AED",fontWeight:T.fw.bold,flexShrink:0}}>확인 <I name="chevR" size={11} color="#7C3AED"/></span>
        </div>;
      })()}
        ...
        <div style={{display:"flex",alignItems:"center",gap:window.innerWidth<=768?3:6,flexShrink:0,flexWrap:window.innerWidth<=768?"nowrap":"wrap",overflowX:window.innerWidth<=768?"auto":"visible",scrollbarWidth:"none",maxWidth:"100%",minWidth:0}}>
          ...
          <button onClick={async (e)=>{
            ...
            try {
              ...
              // 막기 fetch 차단 해제 (세션 회복 시도)
              naverBlockFailRef.current = false;
              await Promise.all([
                ...targets.map(br => naverPollNow(br.naverBizId).catch(()=>null)),
                (async () => { try { await fetchNaverBlockStateRef.current?.(); } catch {} })(),
              ]);
            } catch {}
...
<div style={{marginLeft:"auto",position:"relative",flexShrink:0}} ref={el => { if(el) el._settingsBtn = el; }}>
        ...
        <div className="tl-grid-card" style={{display:"flex",minWidth:"fit-content",position:"relative"}} onClick={handleTlClick}>
          ...
          <div className="tl-time-caster" style={{position:"sticky",left:0,top:topbarH,zIndex:31,width:timeLabelsW,marginRight:-timeLabelsW,height:headerH,alignSelf:"flex-start",pointerEvents:"none",boxShadow:"0 -3px 8px -1px rgba(0,0,0,.12)",clipPath:"inset(-20px 0 0 0)"}}/>
          {/* Time Labels */}
          ...
          {allRooms.map((room, ci) => {
            ...
            // 전 칼럼(막기·미배정·직원) 기본 바탕 흰색 통일 (정우님 2026-06-24) — 미배정 회색 그라데이션 제거
            const colBg = SOFT_BG;
            ...
            return (
              <div key={room.id} className="tl-room-col" data-branch-id={room.branch_id} style={{
                ...
                <div style={{position:"sticky",top:topbarH,height:headerH,marginBottom:-headerH,zIndex:31,pointerEvents:"none",boxShadow:"0 -3px 8px -1px rgba(0,0,0,.12)",clipPath:"inset(-20px 0 0 0)"}}/>
                ...
                <div style={{height:headerH,borderBottom:"1px solid #eee",position:"sticky",top:topbarH,zIndex:10,background:colBg,
                  // 헤더 자체 그림자 전부 제거 (v3.8.58 정우님 "헤더 좌우가 더 진해") — 측면 그림자는 칼럼 본체(.17) 단일 소스,
                    ...
                    return (
                      <div key={block.id} data-rid={block.reservationId||block.id} data-resid={block.id}
                        ...
                        {(() => {
                          ...
                          if (_grpMembers.length < 2) return null;
                          const palette = ["#F59E0B","#10B981","#3B82F6","#EF4444","#8B5CF6","#EC4899","#14B8A6","#F97316"];
                          let _h = 0;
                          for (let i = 0; i < _grpId.length; i++) _h = ((_h * 31) + _grpId.charCodeAt(i)) | 0;
                          const c = palette[Math.abs(_h) % palette.length];
                          return <div title={`동반자 묶음 (${_grpMembers.length}명)`} style={{position:"absolute",top:2,right:2,width:12,height:12,borderRadius:"50%",background:c,border:"1.5px solid #fff",boxShadow:"0 1px 3px rgba(0,0,0,.3)",zIndex:6,pointerEvents:"none"}}/>;
                        })()}
                          ...
                          <div style={{display:"flex",alignItems:"center",gap:2,flexWrap:"wrap"}}>
                            ...
                            {(() => {
                              ...
                              return <span style={{fontWeight:T.fw.bold,color:isNaverCancelled?T.gray500:T.text,textDecoration:isNaverCancelled?"line-through":"none",flexShrink:1,minWidth:0}} title={_isEnDisplay ? `${displayName}${_korFull?` (${_korFull})`:''}` : undefined}>
                                ...
                                {custNum && <span style={{marginLeft:3,fontSize:Math.max(7,blockFs-2),color:T.text,fontWeight:T.fw.bold}}>#{custNum}</span>}
        ...
        const toggleAll = async (block) => {
          ...
          setNaverBlockState(prev => {
            ...
            for (const iid of targets) {
              const cur = items[iid];
              if (!cur || !cur.hour_bit) continue;
              const bits = cur.hour_bit.split('');
              bits[blockSlotPopup.slotIdx] = newBit;
              items[iid] = { ...cur, hour_bit: bits.join('') };
            }
            next[blockSlotPopup.bizId] = { ...(next[blockSlotPopup.bizId]||{}), [blockSlotPopup.date]: items };
            return next;
          });
          ...
          try {
            const r = await fetch('https://blissme.ai/naver-toggle-slot-bulk', {
              method:'POST', headers:{'Content-Type':'application/json'},
              body: JSON.stringify({ biz_id: blockSlotPopup.bizId, item_ids: targets, date: blockSlotPopup.date, time: blockSlotPopup.time, block })
            });
            const j = await r.json();
            const results = j.results || {};
            const failed = targets.filter(iid => !results[String(iid)]?.ok);
            if (failed.length) {
              // 실패한 슬롯만 롤백
              setNaverBlockState(prev => {
                const next = {...prev};
                const items = {...(next[blockSlotPopup.bizId]?.[blockSlotPopup.date] || {})};
                for (const iid of failed) {
                  const cur = items[iid];
                  if (!cur || !cur.hour_bit) continue;
                  const bits = cur.hour_bit.split('');
                  bits[blockSlotPopup.slotIdx] = rollbackBit;
                  items[iid] = { ...cur, hour_bit: bits.join('') };
                }
                next[blockSlotPopup.bizId] = { ...(next[blockSlotPopup.bizId]||{}), [blockSlotPopup.date]: items };
                return next;
