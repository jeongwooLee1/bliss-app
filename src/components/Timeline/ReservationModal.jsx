...
function TimelineModal({ item, onSave, onDelete, onDeleteRequest, onClose, selBranch, userBranches, data, setData, setPage, naverColShow={}, setPendingChat, setPendingOpenCust, betaGroupMode=false }) {
  ...
  // 베타 모드 — 동반자 1명 묶음 등록 (1차: 같은 시간/시술 공유, 직원만 따로 / room_type='separate')
  const [companion, setCompanion] = useState(null); // null | {name, phone, staffId}
        ...
        <div style={{padding:"12px 16px",paddingBottom:_isMob?"calc(12px + env(safe-area-inset-bottom))":"calc(12px + env(safe-area-inset-bottom))",background:T.bgCard,borderTop:`1px solid ${T.border}`,borderRadius:_isMob?"0":"0 0 14px 14px",position:_isMob?"fixed":"sticky",bottom:0,left:_isMob?0:undefined,right:_isMob?0:undefined,zIndex:10}}>
            ...
            {betaGroupMode && !isReadOnly && !isSchedule && f.type === "reservation" && (
              <div style={{padding:"8px 12px",background:"#FEF3C7",border:"1px dashed #F59E0B",borderRadius:8,marginBottom:8,fontFamily:"inherit"}}>
                {!companion ? (
                  <button onClick={()=>setCompanion({name:"",phone:"",staffId:""})}
                  ...
                  <>
                    ...
                    <select value={companion.staffId||""} onChange={e=>setCompanion({...companion,staffId:e.target.value})}
                      style={{width:"100%",padding:"6px 8px",border:"1px solid "+T.border,borderRadius:4,fontSize:13,fontFamily:"inherit",background:"#fff"}}>
                      <option value="">담당 직원 선택…</option>
                      {branchStaff.filter(s=>s.id!==f.staffId).map(s=>(
                        <option key={s.id} value={s.id}>{s.dn||s.name||s.id}</option>
                      ))}
                    </select>
                    <div style={{fontSize:11,color:"#78350F",marginTop:4}}>같은 시간·시술로 묶음 등록 (직원만 따로 — 베타 격리)</div>
                  </>
                )}
              </div>
            )}
