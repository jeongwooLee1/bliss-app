...
export default function BottomNav({ page, setPage, isMaster }) {
  const items = [
    { id:'timeline',     label:'타임라인' },
    { id:'reservations', label:'예약목록' },
    { id:'messages',     label:'메시지함' },
      ...
      {items.map(item => {
        ...
        return (
          <button key={item.id} onClick={() => setPage(item.id)}
            ...
            <span style={{ fontSize:10, fontWeight: active ? T.fw.bolder : T.fw.medium, marginTop:2 }}>
              {item.label}
            </span>
          </button>
        )
