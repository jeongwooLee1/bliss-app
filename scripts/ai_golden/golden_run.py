...
**JSON만** 출력 (설명 금지):
{{"pass": true/false, "lang_ok": true/false, "hallucination": true/false, "reason": "통과/실패 이유 한 문장(한국어)"}}
- pass: must_pass에 적힌 조건만 본다. 그 조건을 모두 충족하면 true. must_pass의 명시 조건을 위반(언어 이탈·작별후 시간재안내·환각·잘못된 동작 등)할 때만 false. must_pass에 없는 추가 기대(예시 형식·길이·이모지 수 등)로는 실패시키지 말 것.
