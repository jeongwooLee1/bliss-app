...
## 현재 버전
- **라이브: v3.7.915** (https://blissme.ai/version.txt) — 직원 이동/근무 팝업 **라이브-시안 1:1 정렬**(정우님 "라이브와 시안 비교해봐" 후): **담당자 교체 맨 아래로**(근무시간 다음 → 푸터 위, segments IIFE 안 이동)·**칩 색점 가시화**(연한 강남데모 color → 진한 팔레트색)·**바 아래 양끝 중복시간 제거**(눈금만). 검증: 빌드OK·babel OK·서연 팝업 스크린샷 시안 거의 1:1. 시안(shot3)과 레이아웃 일치. 상세 CLAUDE.md v3.7.915.
