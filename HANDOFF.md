...
### ⏳ 수정요청 남은 4건 (방향·컨펌 확정, 각 큰 개발)
- **#7 동의서 자동문자(`id_a2kmciryy3`)**: 컨펌=**전지점 자동발송**. `care_sms_scheduler_thread`(bliss_naver.py:2858) 패턴으로 매일 13시 → 동의서 미수령(consent_tokens 발송됨 + customer_consents 미작성) 고객에게 동의서 링크 재발송 문자. `consent_sms_log`류 중복방지 테이블 필요. + 종이동의서 **사진등록**(고객관리 동의서탭 업로드 버튼).
- **#8 고객매칭(`id_qtbeoxor0z`) + `qvz6lolnss`(같은 주제)**: 문자/네이버톡톡 신규 메시지 고객을 기존고객과 연결. 컨펌=**번호일치 + 이름 1개면 자동매칭, 2개+면 직원이 후보에서 선택**. ⚠️ phone 단독매칭 금지룰 준수(번호+이름).
- **`of485ey6bo`**: 지점 문자 발송 **비용을 지점에 정산**(아이디 아닌 지점 전가) + SMS **단문/장문 구별 + 장문 경고** 표시.
