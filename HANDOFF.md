...
## 2026-04-17 세션 2 완료 (v2.2.6 → v2.7.5) — 계정 전환 직전 스냅샷

### 핵심 신규 기능
- **외부 선결제 시스템 통합** (v2.4.0~2.4.3)
  - `sales.external_prepaid/external_platform`, `reservations.external_prepaid/external_platform` 컬럼 추가
  - 네이버 예약금도 외부 선결제로 통일 (18+263건 마이그레이션 완료)
  - 관리설정 → "외부 플랫폼" (AdminExtPlatforms.jsx) — 네이버 시스템 기본 + 추가 관리
  - ReservationModal 예약 탭에 선결제 입력 섹션 (플랫폼+금액), 플랫폼 선택 시 방문경로 자동 매칭
  - SaleForm에서 네이버 예약금 섹션 제거 → 선결제 섹션에 통합, 네이버 예약 자동감지
  - 예약금완료 태그: `isPrepaid || externalPrepaid>0` 둘 다 트리거
  - 방문경로에 "서울뷰티", "크리에이트립" 추가
- **포인트 시스템** (v2.5.0, v2.7.1)
  - `point_transactions` 테이블 신규 (earn/deduct/adjust, sale_id 연결)
  - 고객관리 보유권 탭에 "🪙 포인트" 탭 — 잔액/수동 적립·차감/히스토리
  - SaleForm에 포인트 사용(결제 영역) + 포인트 적립(별도 박스) 분리
  - 시술상품 "포인트 적립" 카테고리 + 3개 쿠폰(10%추가적립·제품전용3만/8만) 추가
- **선불권 차감 히스토리** (v2.5.2)
  - `package_transactions` 테이블 (charge/deduct, won/count, sale_id 연결)
  - 매출 등록 시 다담권·다회권 차감 자동 기록 (담당자·매출연결)
  - 고객관리 보유권 카드에 📜 버튼 → 이력 패널 (수동 차감도 기록)
- **고객번호 자동 발행** (v2.6.4)
  - `bliss_cust_num_seq` 시퀀스 (70000부터) + BEFORE INSERT 트리거
  - 기존 번호 없는 81건 백필
  - SaleForm 랜덤 custNum 생성 제거, DB 트리거가 할당
  - RPC: `next_cust_num()`

### UI 개선
- 매출관리 행에 결제수단별 컬럼(현금/카드/입금/포인트) 표시, 0원도 항상 표시
- 매출 메모는 상세 펼치기에서만 (인라인 편집)
- ReservationModal 모바일: 뒤로가기 헤더 제거 → 우측 상단 ✕ 플로팅 닫기
- 고객정보 팝업 빠른보기 (주용님 요청 반영)
- 지점 필터 localStorage 유지 (유라님 요청 반영)
- 담당자 교체 기능 (지은님 요청 반영)
- 태그 색상 9개 개선 (지은님 요청 반영)
- 시술상품관리 DB 저장 오류 수정 (toDb 누락)
- 하우스왁싱 로고 클릭 → 새로고침
- 예약블록 리사이즈 5분 단위 + 핸들 영역 확대
- 드래그 시 컬럼 이동 우선 (수직 임계치 추가)
- 타임라인 좌/우 엣지 자동 스크롤
- 예약태그 기본 접힘
- + 컬럼에 드롭 시 같은 지점 마지막 직원 컬럼으로 스냅

### 버그/데이터 수정
- schHistory_v1 flat 구조 → 월별 재구성 (1,225건 복구)
- "오늘 휴무" 버튼이 flat 구조로 DB 덮어쓰던 버그 수정
- Realtime 구독 핸들러의 `isModalOpenRef`/`pendingRTQueueRef` 선언 누락 → UPDATE 이벤트 무시되던 핵심 버그 수정 (v2.6.3)
- 15초 폴링 제거, Realtime 재연결 로직 추가
- 2,529 orphan 매출 복구: 2,453건은 전화 매칭으로 재연결, 75건 신규 고객 생성
- 김잔듸 4건 (김시아와 같은 번호) 별도 고객 분리 (전화 매칭 위험성 발견)
- Array.reduce startsWith TypeError 가드 (앱 오류 발생 방지)
- ReservationModal 직원 팝업 중복 렌더링 (branch_id 체크 추가)
- 예약/일정 보호: 예약 있는 컬럼은 "오늘 휴무"/프리랜서 삭제 차단

### 수정 요청 처리 (bliss_requests_v1) — 모두 answered/done
- #유라 지점 필터 유지 (v2.4.4)
- #주용 고객정보 팝업 (v2.4.5)
- #지은 담당자 교체 (v2.4.7)
- #민정 마감 결제수단별 확인 (v2.4.8~9)
- #주용 다담권 차감 이력 (v2.5.2)
- #지은 태그 색상, #주용 고객번호·쿠폰 (자동 답변)

### 현재 버전: v2.7.5

## 2026-04-16~17 세션 완료 (v1.7.0 → v2.2.6)

### 퀵블록 (시도 후 제거)
- 우측 패널 퀵블록 시도 → 사용성 문제로 제거
- QuickBlockPanel.jsx 삭제됨

### + 칼럼 매일 반복 내부일정 템플릿
- + 칼럼 셀 클릭 → 내부일정 태그 선택 → 매일 반복 표시
- 드래그 → 직원 컬럼에 일회성 내부일정 생성
- 데이터: `schedule_data` key=`colTemplates_v1` (`{branchId: [{id, name, tagIds, time, dur}]}`)

### 직원 이동/지원 개선
- 지원/이동 버튼 → **단일 "이동" 버튼** 통합 (시간 없으면 전체, 시간 있으면 부분)
- 여러 지점 연속 이동 가능 (segments 추가 방식, 덮어쓰기 X)
- 이동된 직원 원래 지점 컬럼 **비활성화 유지** (사라지지 않음, 회색 오버레이)
- 비활성 컬럼 내 예약 블록은 클릭/이동 가능
- 각 지점별 **시작~종료 시간 인라인 편집** (select dropdown 즉시 저장)
- **시각적 타임라인 바** — 지점별 색상 + 지점명 표시 + 드래그 경계 조절
- 색상: 밝은 지점색 감지 → 해시 팔레트 자동 적용

### 출퇴근 시간 드래그 조절
- 비활성 영역 경계(출근/퇴근)에 드래그 핸들 추가
- 위아래 드래그 → empWorkHours 즉시 저장

### 오늘 휴무 버튼
- 직원 컬럼 팝업에 "오늘 휴무" 버튼 → schHistory="휴무" 설정 → 컬럼 숨김

### 예약 모달 개선
- 직원 배정 안 되는 버그 수정 (staffId/roomId 매칭)
- 합성 roomId(st_/nv_/blank_) DB 저장 방지 + 기존 116건 정리
- 성별 선택 버튼 추가 (여/남/미지정)
- 고객 정보 줄바꿈 개선 (flexWrap + whiteSpace:nowrap)
- 내부일정 모달 "내부일정" 타이틀 제거
- 반복설정 예약 탭에도 추가 + handleSave 반복 로직 확장
- 근무 외 시간 클릭 시 scheduleOnly 강제 제거 → 예약 탭 전환 가능

### 예약 저장 안정화
- reservation_id NULLS NOT DISTINCT 충돌 수정 → `manual_` prefix 자동 생성
- 반복 예약 각 항목 고유 reservation_id
- isNaverItem 체크에 `manual_` prefix 제외
- 네이버 source 수동예약 isScrapingDone 필터 제외
- 저장 실패 시 개별 재시도 + reservation_id 재생성
- isNaverUnassigned 체크에 staffId 포함

### 타임라인 UI
- 내부일정 블록 "내부일정" 텍스트 제거 (태그만 표시)
- 보유권 유효기간 만료 체크 (note의 유효:YYYY-MM-DD)
- 보유권 pill 그룹화 + 우선순위 정렬 (다담권>재생>토탈>소급)
- 내부일정 시간변경/컬럼변경 시 알림팝업 제거
- 리사이즈 핸들 축소 (24→10px, 중앙 50%)
- 프리랜서 컬럼 삭제 버튼 + 시스템 예약어 보호
- 타임라인 endHour 22→23 (10시까지)
- 직원 컬럼 좌우 이동: 현재 보이는 컬럼 기준으로 동작 (저장된 order만 보던 버그)

### 매출 등록 개선
- 네이버예약금 별도 결제수단 (`gift` 컬럼, "네이버예약금" 표시)
- 다담권 구매 + 오늘 시술 즉시 차감 (customer_packages 자동 생성)
- PKG 패키지 구매 + 오늘 N회 사용 (증감 버튼)
- 신규 구매 시 회원가 자동 적용
- 선불권 카테고리 "다담권(선불이용권)" → "선불권" 리네이밍 + 가격 액면가로

### 매출 관리
- 결제수단별 합계 분리 표시 (현금/카드/입금/포인트 별도 박스)
- 네이버예약금 초록색 표시

### 받은메시지함
- 지점명 이름 앞에 배치 + primary 색상

### 수정 요청 페이지 (BlissRequests.jsx)
- 사이드바 "시스템" 그룹에 "📝 수정 요청" 메뉴
- 직원이 이름+지점+내용+사진 등록
- 관리자가 상태변경/답변/삭제
- 데이터: `schedule_data` key=`bliss_requests_v1`

### 서버 Claude Code 원격 세션
- Oracle Cloud 서버에 Claude Code 설치 (v2.1.69, Opus 4.6 Claude Max)
- tmux 영구 세션 (`tmux attach -t claude`)
- systemd 자동 재시작 (`claude-tmux.service`)
- 모바일 Termius로 SSH 접속 → 세션 이어서 대화 가능
