...
### PENDING — 네이버 예약 블록시간(dur) ≠ 매칭 시술합 (다음 세션 작업, 유저 보류)
**증상** (Tagnipez #1244388060): "Brasilean + Care" 예약 — 시술은 브라질리언(45)+케어(10)=55분으로 잘 매칭됐는데 **블록 dur=80분**으로 어긋남. 표준 `케어` 시술이 10분이라 케어만 잡히면 `max(10,30)=30분`으로 바닥처리되는 케이스도 있음("30분" 증상).
**조사 결과**: 향후 네이버 예약 다수가 block_dur ≠ svc_dur_sum (55·60·80·52·45 제각각). dur 계산(`ai_analyze_reservation` line 2156 `update["dur"]=max(시술합,30)`)이 네이버 슬롯값/스테일값에 덮이는 케이스 존재. `dur`이 `BLISS_PRESERVE_FIELDS`(line 341)에 있어 보존되는 점도 관련.
**유저 승인 방향** ("저장하자, 새세션"으로 보류):
1. 이 예약 즉시 교정: dur 80→55 (19:00–19:55).
2. 시스템 fix: 네이버 분석이 시술 저장 시 블록 dur을 항상 시술합으로 동기화되게 보장(덮이는 원인 제거). **단 직원 수동 시간조정(schedule_log 흔적)은 보존.**
3. 소급: 오늘 이후 네이버 예약 중 dur≠시술합 건 일괄 재동기화(수동조정 흔적 제외) — 유저가 "소급 같이 할지" 확인하겠다 함.
**미결정**: 80의 정확한 출처(네이버 슬롯 vs 이전 매칭 vs 수동) 미확정 — dur preserve 로직 + `ai_analyze` dur PATCH 흐름 추가 추적 필요.
