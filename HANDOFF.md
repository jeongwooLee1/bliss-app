...
## 현재 버전
- **라이브: v3.7.909** (https://blissme.ai/version.txt) — 직원 이동/근무 팝업 **시각 정리**(정우님 "가로 길고 글자 작다"): 근무시간 드롭다운 full-stretch→좌측 컴팩트(flex 0 1 auto, fontSize 14), 라벨 10→12~13px, 세그먼트행 10→12px. 로직 안 건드림(스타일만). 정우님 요청으로 로컬 컨펌 생략·자체확인 후 배포. 상세 CLAUDE.md v3.7.909.
