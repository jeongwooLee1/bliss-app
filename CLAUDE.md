...
- **후속 조정**: 헤더 밴드 연한 라벤더(`#C4B5FD→#DDD6FE`) + 진보라 글씨로 톤다운(흰 글씨는 연한 배경에서 가독성 불가), 로고 흰 타일 제거, `logo-housewaxing.png` 3061px·318KB → 128px·11KB 리사이즈(로딩 지연 해결), 푸터에 매장 전화 `tel:02-515-5141` 링크. ⚠️ 전화번호는 현재 전 지점 공용 하드코딩 — 지점별 분기 필요 시 BRANCHES 맵 확장
