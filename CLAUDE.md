...
- **(2026-06-05 후속) 가격 정합 + 자주답변 추가**: netlify 참고가와 블리스 시술상품(POS) 가격 불일치 발견(하이드라 77k vs 120k 등) → 정우님이 **블리스 시술상품을 정답으로 수정**(하이드라 77,000 등). RAG를 **시술상품 기준으로 재인제스트**(`/tmp/ingest_skincare2.py`, 멱등 — 하이드라 77k/플러스 120k/리버스하이드라 120k/리얼애프터 180k/리버스필링 230k/리버스에이징 250k/천방케어 시리즈 150k~1,000k/에너지 66k·99k·198k/애프터케어 33k·40k). **자주답변(`quick_replies_v1`)에 왁싱·스킨케어 가격안내 한/영 4건 추가**(`qr_price_wax_ko/en`·`qr_price_skin_ko/en`, branchId="" 전체공용, 시술상품 기준). → **시술상품·RAG·자주답변 3곳 가격 일치**. AI 검증: "스킨케어 가격" → 77k 등 정확 응답. (영어권 외국인 "얼굴 피부관리" = "facial"/"facial treatment"/"skin care" — RAG 키워드 포함). 가격 변경 시 ①시술상품 ②RAG 재인제스트 ③자주답변 3곳 동기화 필요.
