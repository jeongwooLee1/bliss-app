...
- **자주답변**: `quick_replies_v1`에 왁싱·스킨케어 가격안내 한/영 4건(`qr_price_wax_ko/en`·`qr_price_skin_ko/en`, 전체공용) — **현재는 정적 텍스트(스냅샷)**라 가격 바뀌면 수동 수정 필요. (동적 가격표 자주답변=services 실시간 토큰 확장은 React 기능, 미구현 — 필요 시 별도). 정적 자주답변 대신 직원이 "AI 답변 추천" 쓰면 항상 최신가(price_table).
