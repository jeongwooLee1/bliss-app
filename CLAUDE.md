...
- **(v3.7.996) 자주답변 UI = 우측 카드 패널**: 입력창 위 드롭다운 → **받은메시지함 오른쪽 별도 플로팅 패널**(`renderQrPanel` createPortal, position:fixed right:16 top:88, z-index 9600). 카드형. **공통/지점별 그룹**: "전지점 공용"(branchId 없음, 보라) 섹션 + 지점별 "OO점 전용"(파랑) 섹션 — 필터는 userBranches 기준(공통=전원, 지점별=해당 지점 보유자만). 관리(추가/수정/삭제)·branchId select 유지. createPortal import 추가(react-dom).
