...
### v3.8.90 — 타임라인 보기전환(일/주/월) 날짜탭 분리 → 툴바 세그먼트 (정우님) (2026-06-13)
...
### RLS 차단 3차 — 남은 전 테이블 완주 (2026-06-14 일요일 밤, 영업외)
2차(결제/금융)에 이어 남은 ~50개 열린 테이블 전수 감사 후 차단. **권위있는 전수 추출**(pg_class+pg_policies로 RLS-off 또는 anon USING(true) 테이블 전체) → 분류 → 차단 → `set role anon` 전수스캔 검증.
- **서버 토큰 커버리지 게이트**: bliss_naver.py `apikey` 헤더 27개 중 26개 `x-bliss-session`(예외=HEADERS 상수[멀티라인이라 같은줄 매칭만 누락, 실제 포함]·aligo API[수파베이스 아님]), ai_booking.py 2/2. HEADERS 상수=`x-bliss-session: _SVCTOK` 확인 + 핵심8(reservations 66·messages 64 읽음) 이미 차단가동 = **서버 전 REST 토큰 커버 확정**.
- **내부 41개 FULL BLOCK** (migration `rls_lockdown_internal_batch_a1` + `..._app_users`): customer_behavior_log·customer_shares·naver_reviews·marketing_campaigns·marketing_sends·account_login_log·ai_active_ack·ai_change_requests·alimtalk_queue·care_sms_log·consent_sms_log·point_expiry_sms_log·reminder_sent_log·reservation_remind_log·chat_booking_state·chat_completed·chat_resolved·blocked_chats·inbox_followup·pending_ai_replies·cust_num_counters·documents·document_chunks·fcm_tokens·fcm_push_log·gemini_usage_log·notification_logs·payment_webhook_log·pkg_audit·reservation_groups·reservation_sources·sales_insight_cache·schedule_data·send_queue·server_logs·sms_send_log·team_chat_messages·bliss_todos·user_requests·onboarding_submissions(서버 /onboarding-submit 경유)·app_users(멤버십—auth_login_v2 DEFINER가 반환, 직접읽기 불요). 정책명 하드코딩 없이 DO블록으로 각 테이블 전 정책 DROP+`bliss_session_<t>` 생성.
- **카탈로그 6개 FULL BLOCK** + **car-watcher am_* 4개 완전잠금**(deny-all, 2026-04-28 사망) + **설정 4개 READ-ONLY-PUBLIC**(businesses·branches·services·rooms = `public_read`(anon SELECT)+`bliss_session_write`(쓰기 토큰)) (migration `rls_lockdown_catalog_carwatcher_config`).
- **r.html(손님 예약확인) 복구**: reservations 차단으로 깨진 페이지 → `get_reservation_public(p_rid)` DEFINER RPC(cust_name·date·time·dur·bid·status·service_id·staff_id·selected_services 최소필드, reservation_id 또는 id 매칭)로 교체. 정적페이지 scp+CF퍼지 배포. 라이브 검증: publishable키(토큰X)로 RPC=데이터 / reservations 직접=`[]` / branches 읽기전용=정상.
...
### 정적 마케팅 페이지 SEO 보강 (내부링크 + OG/Twitter + sitemap) (2026-06-16, 정적 파일만)
멀티에이전트 SEO 감사(루트·정적페이지·robots/sitemap·2026 베스트프랙티스) 결과 기반 보강. **앱/React 무관 — 정적 HTML만 수정, vite build·버전업 없음.**
- **구조 재확인(중요)**: 루트 `blissme.ai/`는 React 앱이 아니라 **`public/landing.html`(풀 프리렌더 정적 마케팅, ~94KB, 빈 root div·React 번들 0)** 를 서빙(nginx `# 루트(/) = 홍보 랜딩(SEO)`, 로그인 토큰 2개 있을 때만 JS로 /timeline 리다이렉트). 앱 셸 `index.html`(30KB)은 /timeline 등 앱 라우트만. 즉 마케팅=정적HTML / 직원앱=SPA 하이브리드 — SEO상 이미 이상적. (과거 HANDOFF에 "루트=React 로그인앱"은 틀린 정보, 정정)
- **왜 정적이 맞나**: 네이버 Yeti 크롤러·카카오/페북 OG봇은 JS 미렌더 → CSR이면 빈 페이지로 인식. 정적이라 구글·네이버·카톡·AI크롤러 전부 초기 HTML에서 콘텐츠 즉시 수신. 작은 SaaS에 SSR/Next 전환은 불필요(실익 없음).
- **수정 7파일**: ① `landing.html` 푸터에 `/pricing.html`·`/about.html` 크롤가능 a href 추가(루트→핵심 마케팅 페이지 내부링크 단절 해소) ② `about.html`·`pricing.html` twitter:card 추가(기존 OG 보유) ③ `privacy/terms/refund.html` OG 풀세트+twitter:card 추가(기존 0) + 푸터 cross-link nav(pricing/about/타 법률페이지) ④ `pricing.html` 푸터 nav(about 링크) ⑤ `sitemap.xml` 전 URL lastmod 2026-06-16 통일·누락 보완.
- 배포: scp 7파일 → /var/www/html/bliss-app + chown + CF files 퍼지(/, landing, about, pricing, privacy, terms, refund, sitemap) + git commit/push. 라이브 전수 검증(루트 footer 링크·twitter·og:type·nav·sitemap lastmod 모두 반영 확인).
- **남은 SEO 권고(미실행)**: ① ★네이버 서치어드바이저 등록·sitemap 제출·색인 확인(한국 노출 핵심, 구글 서치콘솔은 이미 등록·홈 색인됨) ② about.html 본문 보강(~1,269자로 빈약) ③ 무료체험·데모 진입이 JS 버튼이라 크롤 가능 a href 없음(전환 페이지 link equity 미전달). 방문자 카운터(site_visits)는 현재 0 — 외부 유입 자체가 거의 없음 = 네이버 등록의 실익 큼.

### v3.8.119 — 실시간(Realtime) 전면 복구: rt_pings 신호 테이블 단일 채널 (2026-06-16)
RLS 보안 잠금(2026-06-14) 이후 잠긴 7개 테이블의 직접 Realtime이 죽어(anon이 `x-bliss-session` 헤더를 웹소켓에 못 실음 → 이벤트 0건, 폴링 폴백만) 실시간이 끊겼던 것을 **신호 테이블 `rt_pings` 경유 단일 전역 채널**로 복구. 작업세션(worktree `naver-newcust-banner-fix`)이 만든 실시간 복구분을 배포세션에서 main에 반영.
- **메커니즘** (`src/lib/rtPings.js` 신규): 잠긴 테이블(reservations·schedule_data·messages·bank_deposits·team_chat_messages·app_users·ai_change_requests)에 직접 구독 대신, PII 없는 `rt_pings`(anon 읽기 허용, Realtime publication 포함)를 **디바이스당 단일 채널**(`rt_pings_global`)로 구독 → 신호 수신 시 `onRtPing(table, fn)` 콜백이 토큰 인증으로 실제 데이터 **타깃 재조회**. 신호엔 `tbl`+`ref`(예약=날짜, schedule_data=key) 포함 → 해당 날짜/키만 재조회. 채널 18개+→1개라 연결풀 부하는 잠금 전보다 오히려 낮음.
- **DB측(이미 작업세션이 적용, 본 배포 전 검증 완료)**: `rt_pings` 테이블 + 7개 테이블 `rt_ping` 트리거 + Realtime publication 등록 + 5분 퍼지 cron. 전부 존재 확인 후 코드 배포.
- **전환된 곳**: AppShell(예약 동기화·미읽배지·요청배지·공지팝업·입금배지·직원요청·변경요청·공지마퀴) `initRtPings` 1회 + `onRtPing` 8곳 / TimelinePage(예약 날짜별·schedule_data key별·공지) / MessagesPage(받은메시지함 `mergeRecent` — 최근3분 incremental merge) / BankDeposits·SaleForm·useTeamChat·SchedulePage·useData(3훅).
- **머지 방식 = cherry-pick(naive merge 아님)**: 작업브랜치 9커밋 중 **7개는 이미 main에 별도 적용**(선결제·체험단·근무외셀·받은메시지함버튼·날짜탭·대화목록 = v3.8.109~114, 다른 SHA). 실제 신규는 **실시간 2커밋(2488b7f+983d5a5)** 뿐 → 이 둘만 `git cherry-pick -n b0e3e36..983d5a5`로 현재 main 위에 적용(자동병합 성공, 충돌 0). 브랜치 통째 머지하면 main의 v3.8.115~118+beacon+SEO를 되돌려 위험 → 절대 금지.
- **중복 폴링 제거**: v3.8.116의 messages 15초 증분 폴링은 rt_pings가 해결하는 바로 그 "Realtime 끊김"의 임시방편이라 **제거**(MessagesPage). 이제 messages = onRtPing 즉시 머지 + 90초 폴백(mergeRecent). 15초 전역 폴링 잔존은 rt_pings 부하감소 목적과 모순 + 2026-05-23 폴링 과부하 장애 재발 위험이라 정리.
- **배포**: 버전 3.8.119(version.txt+BLISS_V 동시 bump — 클라 reload 유도) + `vite build` + 전체 배포(dist→/var/www/html/bliss-app) + CF purge everything + 라이브 검증(새 번들 200·rtPings 번들 포함·루트 SEO 링크 유지). git commit+push.
- **유의**: ① `AuthContext` 강제로그아웃(session_kill)은 아직 schedule_data 직접 Realtime(죽음) — on-mount 체크로 동작, 별도 트랙 ② `ConsentPanel` Realtime은 이번 범위 밖 ③ 작업브랜치명(`naver-newcust-banner-fix`)은 오해소지 — 실제 내용은 실시간 복구 + 이미 main에 있는 7커밋. ④ 실시간 신호 전달은 라이브 첫 가동이라, 첫 운영시간에 예약 블록 이동·받은메시지·근무표가 새로고침 없이 타 기기에 반영되는지 관찰 권장(폴백 폴링이 받쳐줘 끊김은 없음).

### v3.8.120 — 받은메시지함 UI 3건 (버튼 잘림 / 미읽음 배너 / 카톡 시간) (2026-06-16)
- **① 컴포저 버튼행 잘림** (대표 id_0uho9mcop2): 좁은 docked 패널에서 입력칸 위 버튼(답변추천·번역·예약등록·차단·완료)이 `flexWrap:nowrap;overflowX:auto`라 우측 잘림(데스크탑은 가로스크롤 안 보여 클립) → **`flexWrap:wrap`**(MessagesPage `msg-action-row`)로 줄바꿈. (`.msg-action-row>button{flex-shrink:0}` 유지라 버튼 안 쭈그러듦). 과거 모바일 줄바꿈깨짐 회피용 nowrap이었으나 docked 잘림이 더 큰 문제라 wrap으로.
- **② "답변 안 한 메시지" 배너** (강남 id_xoii3k8w00): ⓐ 라벨이 '답변' 기준 오해 → **"안 읽은 메시지 N건"**(TimelinePage, 실제 기준=is_read). ⓑ 읽어도 안 사라지는 케이스(SMS 등): `markRead`가 `onRead`를 로컬 msgs에 미읽음 있을 때만 호출 → 로컬에 없던 미읽음은 배너 재카운트 트리거 안 됨. **`if(onRead) onRead(unreadCount)` 항상 호출** → AppShell `loadUnreadRef` DB 재카운트 → 대화 열면 채널 무관 즉시 배너 정리. AppShell `onRead`의 `(cnt||1)` 0-decrement 버그도 `if(cnt>0)`로 수정(0 호출 시 사이드바 배지 오감소 방지, 재카운트가 authoritative).
- **③ 카톡 시간 전부 동일** (정우 id_gntzsae2be, 표시만): 카톡 일괄 수집 시 `created_at`(블리스 수신시각)이 같아 시간이 다 똑같이 보임 → **`raw_payload.send_at`(실제 카톡 시각, epoch ms) 우선** 표시. `_dispTime(m)` 헬퍼(kakao면 send_at, 아니면 created_at) → `fmtTime(_dispTime(m))` 4곳(대화목록·버블, compact·full). **브릿지 저장(created_at)·미읽 재INSERT는 카카오 브릿지 세션 별도** — 앱 표시만 수정.
- **검증**: 빌드 OK + 데모 부팅·받은메시지함·대화 열기 콘솔에러 0(핵심파일 AppShell/MessagesPage/TimelinePage 런타임 무크래시 확인). v3.8.120 배포(version.txt 검증, CF 퍼지 everything, 새 번들 200). **landing-staging.html(직원 v2 미리보기)은 dist에 없어 배포 시 백업→복원**으로 보존.
- 요청 3건 `bliss_requests_v1` status=done + 직원톤 답글.

### v3.8.121 — 예약모달 '등록'에서 시술 실행 변경 숨김 (예약 등록 현황만) (2026-06-17)
**문제**(정우): 예약모달 '등록' 블록에 schedule_log 변경 이력이 다 떠서 — 예약 3:20인데 **시술 중 시간 조정이 4:23·4:27로 찍혀** "예약 변경이 4시?"가 되고, **시술 중 추가한 부위(유륜·가슴)**까지 '등록'에 노출. 이건 예약 등록이 아니라 시술 실행 기록 → '등록'은 고객 방문 전 예약 등록 현황만 보여야 함.
**fix**(표시만, 데이터 무변경): `ReservationModal` 등록 블록(line ~2975)에서 schedule_log 줄을 **예약(방문) 시각 이전 것만** 표시. 각 줄의 `[아이콘 MM-DD HH:MM]` 타임스탬프를 파싱해 **예약 datetime(`item.date`+`item.time`) 이전이면 표시, 이후(시술 중/후)면 숨김**. 타임스탬프 없는 줄(⚠ 경고 등)은 유지. schedule_log 저장은 그대로(부위·시간은 매출·블록에 이미 기록 — 손실 0).
...
### v3.8.144 — 내부일정 항목 칩 (NN분) 소요시간 표시 제거 (정우 id_o9ubqvr028 / id_iej3h1hq4z) (2026-06-21)
정우님이 스크린샷으로 두 번 콕 집은 건 — 예약모달 **내부일정(isSchedule) 모드의 "내부일정 항목" 태그 칩**에 붙던 `(NN분)` 소요시간 괄호 표시(출근 20분·이동 60분·청소 30분 등)를 제거.
- `ReservationModal.jsx:2018` 칩 렌더에서 `{tag.dur > 0 && <span>({tag.dur}분)</span>}` 삭제 → 칩에 **태그 이름만** 표시.
...
### AI 일일점검 최근 5일 소급 + 블랙틴팅 FAQ 추가 (2026-06-24, 서버/DB, React 변경 0)
정우님 "어제 말고 최근 5일 검토" → ai_daily_audit.py로 6/19~6/22 소급 점검(6/23은 어제 완료). **AI 대화 31건 중 6건 의심(high 5·low 1), 전부 공지&요청 등록.**
- **패턴**: 6건 중 다수가 "직원이 변경/취소/정정했는데 AI가 무시하고 예약 확정·재확정" 같은 뿌리 → **오늘 추가한 직원 응대중 양보 게이트가 6/23·6/21·6/20 3건을 구조적으로 커버**(한 수정으로 반복 사고 다수 해결).
  - 6/21(가용성 확인 없이 7시 확정 → 직원이 7:30 정정): 실제 대화 확인 — AI 응답 시점에 직원 메시지가 이미 있어 양보 게이트가 막음. 정책 오탐 아닌 진짜 오류였고 이번 fix가 커버. done.
  - 6/20(직원이 취소했는데 AI 재확정): 양보 게이트 커버. done.
- **6/22 (속눈썹 블랙 틴팅·영양제 "된다↔안된다" 번복)**: 학습자료에 정보가 없어 AI가 헷갈림 → **RAG FAQ에 "블랙 틴팅·영양제(에센스) 미제공" 한·영 2청크 추가**(`document_chunks` doc_faq_biz_khvurgshb, gemini-embedding-2/768 RETRIEVAL_DOCUMENT, core=false). end-to-end 검증: 한·영 모두 "진행하지 않습니다" 일관 응답(번복 사라짐). 서버 ingest `ingest_lashfaq.py`(멱등). done.
- **보류(pending)**: 6/19(손님 "내일" 문의를 AI가 "오늘 변경"으로 오해 — 날짜 해석, 별개 트랙·유저 판단 대기) + 6/23 마스카라 중복응답(경미, 유저가 둠).
**유의**: FAQ 추가는 RAG document_chunks만(코드·프롬프트 무관, _rag_search_docs가 자동 주입). 가격은 여전히 시술상품(POS) 단일소스 — FAQ엔 "제공 여부"만. 임베딩은 _rag_search_docs와 동일 모델(gemini-embedding-2)이라 검색공간 일치. ingest 스크립트는 systemd env(BLISS_SESSION_TOKEN) 로드 + business_id 필수(document_chunks not-null). [[reference_bliss_ai_audit]]

### 🚨 공지&요청 데이터 유실 사고 + 빈배열 덮어쓰기 가드 (v3.8.152, 2026-06-24)
**사고**: bliss_requests_v1 value가 `"[]"`(빈 배열)로 통째 사라짐 — 정우님 "공지&요청 리스트가 모두 안 보임". 직전 SELECT엔 53건(staff 47+ai_audit 6) 있었는데 0건.
**원인** (`BlissRequests.jsx`): `loadData`가 자체 fetch로 schedule_data 로드 → **빈 결과(토큰 레이스/RLS 순간 실패/재마운트)** 받으면 `setRequests([])` → 이후 어떤 저장(updateStatus·saveReply·dismiss 등)이든 `saveAll([])`가 그 빈 배열을 DB에 통째 덮어씀. RLS 잠금(2026-06-14) 이후 로드 실패가 곧 데이터 유실로 직결되는 구조. (토큰은 main.jsx 인터셉터가 부착하지만 순간 공백 시 RLS가 []반환 → 정상 빈값과 구분 불가)
**복구**: 04:30 custom dump(`~/.bliss-backup/daily/`, pg_restore=`/opt/homebrew/opt/libpq/bin`)에서 staff 요청 47건 추출 + `ai_audit_log`에서 6/19~23 ai_audit 6건 재구성(done/reply 매핑) = **53건 복원**(`restore_requests.py`, 서버 토큰).
**가드**: `saveAll`·`saveNotices`에 — **빈 배열 저장 시 서버 현재값 재확인(`_serverList`) → 항목이 있으면 저장 차단 + `loadData`로 복원**, 서버 확인 실패 시에도 빈 저장 중단(데이터 보호). `loadData`는 `!r.ok` 시 기존 state 유지(빈 덮어쓰기 방지) + cache:no-store. → 로드 실패가 데이터 유실로 이어지지 않음.
**유의**: 가드는 "빈 배열로 전체 덮어쓰기"만 차단(정상 단건 삭제로 0건 되는 극히 드문 케이스도 차단되나 데이터 안전 우선). 같은 패턴(자체 fetch + 빈결과 setState + save)이 다른 RLS 잠금 키 컴포넌트에도 있을 수 있어 점검 대상. pg_restore는 PATH에 없어 libpq 경로 직접 사용. 백업은 매일 04:30이라 그 이후~사고 사이 신규 staff 요청은 복구 불가(소량 가능).

### 서버 — AI: 손님이 '다른 날' 물으면 기존(오늘) 예약 앵커링 금지 (6/19 사고 fix) (2026-06-24, React 변경 0)
정우님 "고쳐"(6/19 ai_audit). **사고**(WhatsApp Austin, 6/19): 손님이 오늘 예약 노쇼(교통) 후 **"내일 빈자리 있어요?"를 3번** 물었는데, AI가 계속 **"오늘 3시 왕십리 어떤 시술 원하세요?"** 로 기존 오늘 예약에 앵커링 → 손님 "내일" 의도 무시. (직원의 한국고객 노쇼 규정 외국인 오적용·반복멘트는 직원 응대 가이드 이슈로 별개)
**fix** (`ai_booking.py` 기존예약 프롬프트 블록): 기존 `_ex_time_passed`(지난 예약 재안내 방지) 룰에 더해, **"손님이 다른 날(내일/tomorrow/모레/특정일)의 빈자리·예약을 물으면 기존 예약 날짜·시각·시술로 끌어오지 말고 그 날짜 기준으로 안내(가용문의=답만, 예약의사 분명=그 날짜로 진행), '오늘 N시에 어떤 시술?' 되묻기 금지"** 룰 추가. 백업 `ai_booking.py.bak_otherday_*`, restart.
**골든**([[feedback_bliss_ai_golden_gate]]): 신규 케이스 `other_day_no_anchor`(기존 오늘 예약 + 손님 "내일 빈자리?" → 내일 기준 응답·noop). **32/32 게이트 PASS**(회귀 0). 단 첫 run에서 ① other_day 심판 오탐(기존예약 언급을 환각으로 판정 — must_pass 재보정으로 해결) ② couple_booking noop 플레이크(커플 예약금 선요구 vs 즉시예약 모델 변동 — 재run 통과, 내 변경과 무관 경로). golden_run.py(staff 역할+케이스별 suggest_only)·golden_set.json git+서버 동기화.

### 🚨 공지&요청 재유실 — 옛 버전 탭이 재발시킴 (가드는 신버전만) (2026-06-24)
v3.8.152 가드 배포 + 53건 복구 직후, **정우님 브라우저의 옛 버전(3.8.151, 가드 없음) 탭**이 또 빈 배열로 덮어써 재유실(13:09). 가드(saveAll 서버 재확인)는 신버전에만 있어, 새로고침 전 옛 탭이 재발시킴. → 재복구(53건, restore_requests.py 재실행, 6/19 done 반영). **정우님이 3.8.152로 새로고침(또는 버전 자동 리로드)하면 가드가 영구 차단.** 그 전까진 옛 탭이 또 비울 수 있어 새로고침 필수 안내.

### v3.8.153 — 모바일 주(week)뷰 가로 스크롤 안 됨 fix (배포세션 인계, 강남 id_jy9cvkdlrf) (2026-06-24)
작업세션 인계 패치(`/Users/cripiss/TP005/calview_week_hscroll.patch`) 배포세션이 검증·적용·배포.
**증상**: 모바일 주뷰에서 21~23일만 보이고 24~27일로 우측 이동·스크롤 안 됨.
**원인**(`CalendarViews.jsx` WeekView): 주뷰 그리드 `repeat(7,1fr)` + 컨테이너 overflow가, 예약칩(whiteSpace:nowrap)이 화면폭 초과 시 우측 4일을 잘라 스크롤도 막음.
**fix**: 그리드 `repeat(7,1fr)` → **`repeat(7, minmax(120px,1fr))` + `overflowX:auto, overflowY:hidden`**(line 167). 모바일(~390px)은 7×120=840px 오버플로 → 가로 스와이프로 일주일 전체. 데스크탑은 1fr이 채워 스크롤 없음(무회귀). month 뷰·일뷰 무영향.
**배포 점검**: 루트에 patch 4개였는데 3개(empday_order·kakao_sender_badge·noshow_cancel_decrement)는 **이미 v3.8.142에 적용된 잔재**(apply --check 실패로 확인) → 미적용. 신규 calview 1개만 적용. 배포 후 stale patch 4개 정리. [[feedback_bliss_deploy_check_patches]]

### v3.8.154 — 공지&요청 처리완료 분리 필터 + reviewing 정리 (배포세션 인계) (2026-06-24)
정우님 요청: 처리완료(done)를 목록에서 분리. 작업세션 인계 패치(`blissreq_done_filter.patch`) 배포세션이 검증·적용·배포.
- **필터 칩**(`BlissRequests.jsx` 요청 목록 상단): **[진행중][처리완료][전체]** + `reqFilter` state 기본 `active` → **done 숨김**(진행중=pending+reviewing만 표시). 칩에 건수 표시. 빈 상태 메시지 필터별 분기. 렌더만 변경 — 저장 로직(v3.8.152 데이터유실 가드 `_serverList`) 무영향(4개 매치 보존 확인).
- **reviewing 정리**(작업세션 DB 처리): 7→5. **done 2건** — 리뷰 AI 자동답변(id_gnx2s1ax9e, 코드 구현완료) · 데모 관리설정(id_qpd03i8bzk, 대표권한 정상). **keep 5건**(실제 대기): 강남 채팅 고객메모(미구현 feature)·강남 령은 근무시간 회색(재현 필요)·강남 조은빛 고객변경 매출번호(데이터 조사)·수연 내부일정 메모 되돌림(재현 진단중)·정우 모바일 예약 줄바꿈(재현 필요). 함부로 일괄 close 안 함.
**유의**: calview_week_hscroll은 직전 v3.8.153에 이미 배포·done — 작업세션 보고의 "대기 2개"는 그 사이 생성된 stale(calview는 무시). 배포 후 패치 정리.

### 🚨 v3.8.155 — 로그인 직후 빈 타임라인 fix (매 로그인·전 업체 영향, 최우선) (2026-06-24)
정우님 보고: "아이디 바꿔서 들어가면 다 이러네" — 모담뿐 아니라 **하우스왁싱 포함 전 업체, 매 로그인/계정전환마다 타임라인 직원칸이 안 뜸**(빈 화면). 어제 진단(로딩표시 누락)은 불충분했고, 실제는 **stuck 빈칸**(새로고침 전까지). 작업세션 근본원인 규명 + 패치.
**근본원인** (`TimelinePage.jsx` schHistory 로드 useEffect): ① deps `[]` → 마운트 시 1회만 fetch ② `_activeBizId`는 React state 아닌 **db.js 모듈변수** → 계정 전환에도 재실행 안 함 ③ 로그인 직후 세션토큰/biz 준비 전 그 1회 fetch가 RLS(6/14 잠금)로 빈 배열 → schHistory 빈 채 고정 → `getWorkingStaff` 전부 null(1004) → 직원칸 0 → 빈 타임라인. **재시도 없어 새로고침 전까지 영구 빈칸.**
**fix**: schHistory 로드를 `_loadInit`로 감싸 **빈 결과/실패 시 650ms × 최대 6회(~4초) 재시도**(토큰·biz 준비되는 순간 로드) + fetch에 `x-bliss-session` 명시 부착(인터셉터 타이밍 방어) + `_cancelled` cleanup 가드. 바운드 재시도라 무한루프·악화 위험 0. 배포세션 단독 배포(검증 격리).
**⚠️ 검증 필요**: worktree에서 로그인 흐름 직접 테스트 불가 → **정우님 계정 전환(로그아웃→재로그인)으로 직원칸 즉시 뜨는지 1회 확인 필요.** 안 되면 폴링경로(30초)·다른 로드요소까지 재시도 확대.
**대기 패치 4건**(다음 배포): timeline_move_save_retry(수연)·timeline_earlystart_gray(령은)·resmodal_mobile_wrap(정우 모바일)·chat_customer_memo(강남) — reviewing 항목들.

### 🚨 v3.8.156 — 로그인 빈 타임라인 진짜 fix: employees(직원목록) 로드도 재시도 (2026-06-24)
v3.8.155(schHistory 재시도)로도 모담·하우스왁싱 로그인 빈 타임라인 **여전히 재현**(정우님 "안된다"). 원인 = **employees(`empList`) 로드도 동일 버그였는데 v3.8.155는 schHistory만 고침.**
- `TimelinePage.jsx` 직원목록 useEffect(534): deps `[]` + 토큰없음 + **빈 결과여도 재시도 안 함** → 첫 로그인 토큰/biz 준비 전 fetch가 RLS 빈배열 → `empList=[]` 고정 → BASE_EMP_LIST 0 → 직원칸 0(schHistory가 떠도 무의미).
- DB 확인: 모담 employees_v1 193B·schHistory 785B / 하우스왁싱 4KB·61KB **둘 다 데이터 있음**(빈 게 정상 아님 = 진짜 버그).
- **fix**: employees 로드를 `_loadEmp`로 감싸 **빈 결과/실패 시 650ms×6 재시도** + `x-bliss-session` 명시 + `_empCancelled` cleanup 가드. schHistory(v3.8.155)와 동일 패턴. 재시도마다 `${_activeBizId}` 재평가라 biz 타이밍도 커버.
- 이제 첫 로그인에 **empList + schHistory 둘 다** 토큰·biz 준비되는 순간(최대 ~4초) 로드 → 직원칸 표시.
**⚠️ 검증**: 여전히 worktree 로그인 테스트 불가 → 정우님 로그아웃→재로그인으로 직원칸 즉시 뜨는지 확인 필요. (이번엔 양쪽 다 고쳤으니 될 것)

### 🚨 v3.8.157 — 업체 전환 시 빈 타임라인 진짜 fix: viewBids 타업체 stale (2026-06-24)
v3.8.155/156(schHistory·employees 재시도)로 **하우스왁싱은 해결**됐으나 **모담은 여전히 빈칸**(정우님 "모담 안됨", 단 "새로고침하면 뜸"). 모담 데이터(지점 br_id_x316ludvqq·직원 원장로운·근무표 6/24) **다 정상 존재** = 진짜 버그.
**근본원인** (`TimelinePage.jsx` viewBids): `viewBids`(표시할 지점)가 **localStorage(`bliss_timeline_viewBids_v1`)에 업체 공유**로 저장. 업체 전환 시 ① userBranches 동기화(1423)가 `isMaster ||`로 **저장값 무조건 keep** → 하우스왁싱 지점 id 유지 ② 지점변경 동기화(1437)가 그 id들을 모담 지점으로 필터→전부 안 맞아 **빈 배열** → branchesToShow 0 → 칼럼 0. 새로고침하면 localStorage가 []로 바뀌어 기본값(모담 전체)으로 떨어져 떠짐(="새로고침하면 뜸"의 정체).
**fix**: ① userBranches 동기화 검증을 `isMaster ||`(무조건keep) → **현재 업체 지점(allBranchList)에 속하는지**로 변경(타업체 id면 reset) + deps에 allBranchList.length 추가. ② 지점변경 동기화에서 필터 결과 비면 `defaultViewBids`(현재 업체 기본값)로 reset.
**검증(프리뷰 재현)**: demo에서 viewBids를 타업체 id로 stale 세팅 후 리로드 → fix가 demo 지점으로 reset + 직원칸 6개 정상 렌더 확인(fix 없으면 0). 모담 시나리오 동일.
**유의**: 빈 타임라인은 원인 2종이었음 — (A) 데이터 로드 token/biz 타이밍(v3.8.155/156 재시도, 하우스왁싱 해결) + (B) viewBids 타업체 stale(v3.8.157, 모담 해결). 둘 다 필요. 정우님 모담 로그인으로 직원칸 즉시 뜨는지 최종확인 요청.

### v3.8.158 — reviewing 4건 일괄 fix (수연·령은·모바일모달·채팅메모) (2026-06-24)
빈 타임라인(v3.8.157) 해결 확인 후, 대기 패치 4건 검증·배포(배포세션). 전부 apply-clean(내 TimelinePage 수정과 라인 안 겹침).
- **timeline_move_save_retry** (수연 id_rkappwqb6s, 내부일정/블록 이동 되돌림): 이동 저장이 fire-and-forget `.catch(console.error)`라 실패 시 조용히 유실→블록 원위치. → **await + 1회 재시도 + 실패 시 옵티미스틱 원복**(서버와 일치).
- **timeline_earlystart_gray** (령은 id_eyf3gb9fjl, 조기출근 회색): 근무외(회색) 오버레이를 영업시작/마감으로 자르던 것 → **직원 근무시간 기준**(클릭 판정과 동일)으로 변경. 조기출근·잔업 시간 흰색 유지.
- **resmodal_mobile_wrap** (정우 id_eo56g0rafo, 모바일 예약모달 줄바꿈): 고객정보 버튼행 모바일 레이아웃 줄바꿈/wrap 정리(2262~2495 영역, 9헝크 스타일).
- **chat_customer_memo** (강남 id_wwdmkla7o0, 채팅 고객메모): 받은메시지함 대화 헤더에 **고객 메모(특이사항) 작성/확인** 추가 — `customers.memo` 저장+setData 동기화, 연결된 고객만. 미구현 feature였던 것 신규 구현.
**유의**: resmodal는 모바일 레이아웃이라 정우님 모바일 예약모달 확인 권장. chat_memo는 customers.memo 전체 덮어쓰기(기존 memo 프리필 후 편집이라 보존). reviewing 5건 중 4건 done, 조은빛 매출번호(데이터 조사)만 남음.

### v3.8.159 — 데이터유실 버그부류 선제 점검·하드닝 (reportAiError·quick_replies) (2026-06-24)
오늘 2번 터진 버그부류(① 첫로그인 빈결과 로드→state[] ② 빈배열 통째 덮어쓰기 유실)를 전 schedule_data 쓰기 사이트 전수 점검. RLS 잠금(6/14) 이후 잠재 유실 지점 추가 발견·수정.
- **점검 결과**: SchedulePage 근무표 저장(`saveSchHistory`)=DB 최신값 재조회+deep-merge라 **안전**(catch 폴백만 미세). TimelinePage 대부분 맵/단일객체 저장. BlissRequests=v3.8.152에 가드 완료. → 추가 위험 2곳:
- **reportAiError** (MessagesPage 🚨오류신고 버튼, **라이브**): bliss_requests_v1 재조회 후 `[row,...list]` 저장인데, 재조회 빈결과(토큰레이스)면 `[row]`만 저장→**전 요청 유실**. → 빈결과면 0.5초×4회 재시도 후 append(레이스 시 항목 회수). ⚠️ CLAUDE.md엔 "버튼 제거"라 적혀있었으나 실제 2511줄 활성이었음.
- **quick_replies(자주답변)**: `loadQuickReplies` 빈결과→state[]→`persistQuickReplies` 저장이 기존 답변 통째 덮어씀(BlissRequests와 동종). → 로드 빈결과 700ms×5 재시도 + 저장 시 빈 목록이면 서버 재확인·차단·재로드.
**유의**: 동종 버그부류 = "RLS 잠금 schedule_data를 첫로드 빈결과로 받아 state 굳음 → 그 state로 저장 시 유실". 신규 schedule_data 배열blob 로드/저장 추가 시 같은 가드(재시도+빈저장 서버재확인) 필수. SchedulePage 로드는 첫로그인 빈표시 가능하나 저장이 merge라 유실 아님(저우선순위).

### v3.8.160 — QuickRequest(우클릭 수정요청)도 동일 가드 (bliss_requests 유실 방지 완결) (2026-06-24)
QuickRequest.submit(전 직원 우클릭 수정요청, common/QuickRequest.jsx:62)도 reportAiError와 동일 — bliss_requests_v1 재조회 후 `[row,...list]` 저장인데 재조회 빈결과(토큰레이스)면 `[row]`만 저장→전체 유실. 빈결과면 0.5초×4회 재시도 후 append.
**→ bliss_requests_v1 클라 쓰기 3곳 전부 가드 완료**: BlissRequests saveAll/saveNotices(v3.8.152) + reportAiError(v3.8.159) + QuickRequest(v3.8.160). quick_replies도 가드(v3.8.159). (ai_daily_audit.py 서버 쓰기는 service 토큰 항상 유효라 저위험.)

### v3.8.161 — 재방문 스탬프 제도 전체 구현 (엔진 v2 + 매출등록 적립 + 보상 + 셀프조회 + 관리설정) (2026-06-24)
기획서(블리스미 재방문스탬프) 풀 구현. **제도 기본 OFF** (관리설정에서 매장이 켬 → 그 전까진 라이브 무영향).
- **엔진 v2 (DB, migration `stamp_program_v2_config_milestone`)**: `accrue_stamp(p_biz,p_cust,p_sale_id,p_visit)` — `businesses.settings.stamp_program` 설정 기반(on/off·windowDays·milestones 서버가 읽음, 클라는 호출만). 28일 자동판정(직전 방문일 간격) + 멱등(같은 매출/같은 날 중복방지) + 사이클 리셋(28일 초과 or final 회 완성) + **마일스톤 자동 감지**(도달 회차의 보상 1회만, milestones_given 추적). `get_stamp(p_biz,p_cust)` 조회 RPC(적립 없이 현황). 둘 다 SECURITY DEFINER(customer_stamps RLS 잠금 우회) + anon grant. 6 시나리오 + 마일스톤 검증.
- **SaleForm 적립 hook** (`SaleForm.jsx`): 매출 insert 직후 — 시술 방문(svcTotal>0 or 보유권사용 or 시술체크 or 추가시술)일 때만 `accrue_stamp` 호출(editMode 제외, try/catch로 매출저장 비차단). 마일스톤 도달 시 **보상 쿠폰 자동 발급**(customer_packages, rewardServiceId 있으면 그 시술 무료1회권/없으면 라벨쿠폰, 유효 2개월).
- **SaleForm 진행 표시**: 고객 선택 시 `get_stamp` 로드 → 헤더 아래 배지 "재방문 스탬프 N/8 → 이번 시술 시 M회차" + 마일스톤 예정 시 "🎁 {보상} 발급"(28일 간격 클라 추정, 실제 적립은 저장 시 서버 확정).
- **관리설정 UI** (`AdminStampProgram.jsx`, 사업장 관리 → "재방문 스탬프", slug stamp-program): on/off + 재방문 인정기간(일) + 보상 회차 편집(회차n·보상명·보상시술 select·사이클완성 체크·추가/삭제). final = 사이클완성 회차 자동. 멀티테넌트(settings 기반, 하드코딩 0).
- **고객 카톡 셀프조회** (서버 `/customer-verify-check` + `public/mypage.html`): SMS 인증 후 보유권·포인트에 더해 **스탬프 ○/8 시각화**(채움/보상★ 도트 + "N번 더 방문하면 {보상}"). 서버는 get_stamp RPC 호출(stamp 필드 추가). 프리뷰 렌더 검증(3/8·보상3·다음보상 안내).
- **미구현(후속)**: 25일 이탈방지 알림 / 시술완료 시 28일 뒤 예약 자동추천 / 28일내 재방문율·완주율 지표. (기획서 자동화 ③의 일부)
**유의**: 제도 ON 전까지 accrue_stamp/get_stamp 모두 `{on:false}` no-op. 보상 쿠폰은 customer_packages(보유권)에 발급 → 직원이 다음 방문 매출등록 시 적용. rewardServiceId 미지정 보상은 라벨 쿠폰(직원 수동 무료처리). 적립은 SaleForm 매출등록 경로만(오라클 과거 임포트·페널티 매출 미적립 — 의도). 동종 schedule_data 유실 부류 아님(customer_stamps 전용 테이블·RPC upsert).

### v3.8.162 — 재방문 스탬프 보상 = 혜택관리 쿠폰 재사용 (옵션2, 설정 일원화) (2026-06-24)
정우님 "왜 또 따로 설정을 만들었어 — 이벤트 등록(쿠폰) 재사용하면 되잖아" 지적 → 옵션2(보상만 통합) 반영. **카운터 엔진(accrue_stamp, 28일 회차)은 이벤트 엔진에 없는 기능이라 그대로 유지**(이벤트 엔진은 per-sale 트리거뿐, 방문 간격·N회차 카운터·사이클 리셋 개념 없음 — 코드 확인). 통합은 **보상 발급 쪽만**.
...
### v3.8.169 — 공지요청 폴링 + 예약경로 빠짐 경고 + 알람음 끄기 버튼 (2026-06-25)
워크트리 인계 patch(req_polling_route_alarm, 4파일 additive). 정우 3건 묶음.
- **공지&요청 자동 갱신**(정우 "풀링 안 됨", BlissRequests): 25초 폴링으로 새 요청/공지가 새로고침 없이 목록 표시. 편집·작성·답글 중(editingDescId/editingNoticeId/showForm/showNoticeForm/showOffForm/replyText)엔 스킵(입력 방해·빈배열 덮어쓰기 방지).
...
### AI 자동점검 3건 검토·처리 + 직원응대중 "all set" 확정 보류 보강 (2026-06-26, 서버, React 변경 0)
정우님과 6/25 AI 자동점검(ai_audit) 3건 1:1 검토. **대부분 어제(6/25) 수정 전 새벽 대화를 잡은 것**:
- **#1 (심각, Ken Wells WhatsApp)**: 직원 응대 중 AI가 이전 시간(20:00)으로 재확정 → **이미 어제 수정됨**(ai_booking.py:3566 직원응대중 확정멘트 보류 가드, "Done!…confirmed"가 키워드에 걸림). done.
- **#2 (보통, LINE 영업시간 7시)**: 7시 제안은 **직원이 직접** 한 것(AI 아님)=오탐 + 어제 영업시간 게이트(11:00~20:30) 추가됨. done.
...
### v3.8.173 — 공지&요청 첨부 이미지 클릭 확대(라이트박스) (정우 id_b365mo9rxm 연계) (2026-06-26)
정우님: "2번(강남 무료쿠폰 매출등록 안됨)에서 등록한 이미지를 확대해서 볼 수 있게" — 공지&요청 첨부 이미지를 크게 보는 기능 추가.
- 신규 `ImageLightbox`(BlissRequests.jsx 모듈 컴포넌트, `createPortal`로 body 마운트): 클릭/휠로 줌(1↔2.5x, 휠 6x까지), 확대 시 오버레이 스크롤로 이동, 바깥 클릭·ESC·× 버튼으로 닫기, 안내문구. z-index 99999.
- 요청 카드 첨부 이미지(이전 클릭 불가)에 `onClick={setLightbox}` + cursor:zoom-in. 공지 이미지는 기존 `window.open(새 탭)` → 인앱 라이트박스로 교체(모바일에서도 매끄럽게).
- **검증**(데모, preview): 데모에 테스트 이미지 첨부 요청 임시 삽입 → 카드 펼침→이미지 cursor:zoom-in 확인 → 클릭→라이트박스 오픈(z99999·다크오버레이·확대 이미지·닫기버튼·안내문구, 스크린샷 확인) → ESC 닫힘+body 스크롤 복원 확인 → 테스트 데이터 삭제. 빌드·콘솔 에러 0.
- **유의**: 라이트박스는 요청·공지 표시 이미지 둘 다 적용. 요청 카드는 펼침(openId) 시 첨부가 보임(기존 동작). #2 본 건(강남 무료쿠폰 매출등록 실패)은 이 확대로 정우님이 첨부 화면을 크게 보고 원인 진단하기 위함 — 매출등록 코드 자체 수정은 화면 확인 후 별도.

### v3.8.174 — 네이버 "확정 반영"(이미 확정된 예약 즉시 동기화) + 보유 쿠폰 클릭 시 대상 시술 자동 체크 (2026-06-26)
정우님 수정요청 2건 묶음 배포.
- **A. 네이버 확정 안 됨 fix (ReservationModal)** — 남창용 #1275420545 사례: 방금 들어온 재방문 네이버 예약을 직원이 "네이버 확정" 눌렀는데 7번 전부 실패(서버 로그 ok=False "네이버에서 아직 확정 처리 전" = 갓 생성 예약을 네이버 확정-액션 API가 몇 분간 거부, RT47류). 단 네이버 예약정보엔 이미 확정일시(15:03)가 있었음(네이버 자동확정). 4분 뒤 자동 재스크랩으로 reserved 풀림. → fix: `onConfirm`에서 **`f.naverConfirmedDt`(또는 item)가 이미 있으면** 실패하는 네이버 확정 API 호출 없이 **바로 status=reserved 동기화 + return**. 버튼 라벨도 그 경우 "✓ 확정 반영"(네이버 미확정 일반 예약은 기존 "✓ 네이버 확정"=실제 API 호출). 재방문 자동확정 예약 확정버튼 실패 루프 제거.
- **B. 보유 쿠폰 클릭 → 대상 시술 자동 체크 (SaleForm, 강남 id_b365mo9rxm)** — 보유 쿠폰 패널(v3.7.216) `onClickCoupon`이 **대상 시술 1개일 때만** 자동 체크하고 다중 대상은 무동작(안내만)이던 것 → **매칭되는 대상 시술 전부 클릭 한 번에 자동 체크**(단·복수 모두). targetSvcs는 실재 시술만(`.filter(Boolean)`)이라 매칭 안 되는 쿠폰은 자동 제외("안 되는 건 빼고"). cursor pointer·안내문구("→ 클릭하면 시술 N개 자동 추가") 통일. `toggle`은 functional setItems라 루프 안전, viewOnly/editMode 무동작. free_service 쿠폰은 체크된 대상 시술만 무료라 다중대상 클릭 시 다 무료(보통 단일대상이라 무관, 한개만 쓸 땐 직원이 해제).
- **검증**: 빌드 통과. A는 모달 클릭 E2E가 프리뷰 환경 한계(뷰포트·타임라인 모달)로 화면검증 못 함 — 라이브 스팟체크 권장(naverConfirmedDt 필드 존재·주입은 확인). B는 단일대상(실제 캡처 케이스)은 기존과 동일 동작이라 무회귀, 다중대상만 확장.

### 서버 — AI 예약 시간변경 = 취소+재생성 → 기존 예약 in-place UPDATE (대표 id_n5c7x7az58) (2026-06-26, React 변경 0)
**증상**(대표, Ken Wells WhatsApp): 6:30→7:00 시간변경을 AI가 처리하면 **기존 예약을 취소하고 새 예약(새 reservation_id)을 생성** → 타임라인에 "[AI예약변경]" 새 블록 + **변경로그 없음** + 취소된 중복 레코드 누적(Ken Wells 06/26 한 명에 취소 6건). 대표 요청: "시간변경은 기존 예약을 변경하는 것으로(새로 등록 X) + 예약모달 변경로그 기입".
**원인**: `create_booking_from_ai`가 `find_existing_booking`으로 기존 예약 찾으면 → `cancel_booking(기존)` + 새 rid로 INSERT (memory "예약 변경=취소+재생성"). 변경 이력(schedule_log) 미기록 + reservation_id 바뀜 + 취소 찌꺼기.
**fix** (`ai_booking.py`, 백업 `bak_inplace_20260626_075851`): 기존 예약 발견 + 동일아님(변경)이면 → **취소 안 하고 in-place UPDATE**. ① `_update_id=existing.id`, `_change_from=기존 날짜·시각` 기록(취소 제거) ② `rid = _update_id or _gen_id()`(변경이면 기존 id 유지) ③ INSERT 분기: `_update_id` 있으면 POST 대신 **`reservations?id=eq.{기존}` PATCH** — `date/time/memo/schedule_log` + (AI가 시술 줬을 때만)`selected_services/tags` 갱신, **dur·staff_id·room_id·status는 미포함=보존**(블록이 같은 직원 칼럼에서 시간만 이동). `schedule_log`에 **`[AI 변경] {old} -> {new} (고객 요청)`** prepend + 기존 로그 보존(GET으로 읽어 합침). 커플 동반자 생성은 `not existing` 가드라 변경 시 미생성(중복 방지) 유지.
**검증**: ① **직접 동작 테스트**(테스트 예약 생성→시간변경 호출→정리) — 같은 id 수정(새 예약 0)·time 14:00→16:00·시술/dur(120)/직원(소연)/status(reserved) **보존**·schedule_log "[AI 변경] … + 기존로그 보존" 전부 **PASS** ② **골든게이트 37/37 PASS**(재실행 — 변경은 DB쓰기 경로라 골든이 create_booking 통째 모킹·결정로직 무영향, price_male_complex 1회 플레이크는 재실행 통과로 회귀 0 확인) ③ 서버 재시작 active.
**유의**: time-only 변경 시 AI가 시술 미반환이면 selected_services 미갱신=기존 보존(시술 안 날아감). dur는 변경 시 항상 보존(기존값). status 보존(reserved면 reserved 유지 — 손님에게 이미 "확정" 안내했으므로 일관). 기존 Ken Wells 취소 찌꺼기 6건은 hidden(cancelled)이라 방치(과거 이력). 커플 예약(동반자도 같은 chat_user_id)의 변경은 find_existing이 리더/동반자 중 최근 건 매칭 — 엣지(이번 범위 외). ai_booking.py는 서버 전용(git 미추적) → 앱 버전업·CF퍼지 무관.

### 서버 — LINE 채널 AI 자동응답 연결 (주소 등 문의 무응답 fix) (2026-06-26, React 변경 0)
**증상**(정우 id_3jntjz2evq): 라인 손님(Mekaella)이 06/25 17:59 "홍대점 정확한 위치?" 물었는데 AI 무응답 → 다음날 09:34 직원이 수동 답변(15시간). "주소 같은 건 AI가 자동 응대해야".
**근본원인**: `bliss_naver.py` `_proc_line_event`(LINE 웹훅)이 메시지 **수신·번역만** 하고 **`ai_booking_agent`를 호출 안 함** → LINE은 AI 자동응답이 아예 미연결(WhatsApp/Instagram/네이버/SMS는 호출함). 그 대화의 10:26 "AI 자동응답"은 실은 직원 ✨추천(manual, is_ai=true)이었음. delay/schedule/pending_ai_replies 인프라는 채널 무관(워커→send_queue→`_send_line_message`)이라 멀쩡 — 진입(웹훅→에이전트)만 빠져 있었음.
**fix** (`bliss_naver.py` `_proc_line_event`, 백업 `bak_lineautoreply_20260626_082659`): 메시지 INSERT + 번역 스레드 다음에 **AI 자동응답 스레드 추가**(WA 패턴 그대로) — text 메시지면 `ai_booking_agent(text,"line",user_id,"line")` → `_ai_decorate("line",…,channel="line")`(직원개입 5분 pause·AI prefix) → 비어있지 않으면 `send_queue`(channel=line, is_ai=true) INSERT. 미디어/스티커(`[..]`)·직전 out(이미 답변) 스킵. 영업시간엔 ai_booking_agent가 delay 분기로 pending_ai_replies enqueue(3분 후 워커가 send_queue로 발송), 야간/active는 즉시 — 다른 채널과 동일 동작.
**검증**: ai_booking_agent("홍대점 주소 알려주세요","line",force,suggest_only) → "하우스왁싱 홍대점 주소는 서울시 마포구 양화로 178 목화빌딩 5층입니다 😊"(직원이 다음날 수동 답한 그 주소) 정상 응답 + py_compile + 재시작 active. 골든 무관(에이전트 결정로직 무변경, LINE 웹훅 진입만 추가).
**유의**: LINE도 이제 WA/IG와 동일하게 채널설정(ai_auto_reply_channels.line=true)·스케줄(야간 21:00~09:30 즉시 / 영업시간 3분 delay)·직원응대중 게이트 적용. account_id="line" 고정(LINE 단일 채널, biz_khvurgshb 전용). 발송·아웃고잉번역·echo는 기존 send_queue_thread가 처리.

### v3.8.175 + Edge Functions — 월 구독료 정기결제 토스 빌링 → 포트원 V2 + KCP 전환 (2026-06-26)
토스 자동결제(빌링) 계약이 막혀(BILLING_READY=false 였음) 월 이용료 정기결제가 멈춰 있던 것을, **포트원(PortOne) V2 + NHN KCP(kcp_v2) 카드 빌링**으로 전환·활성화. 충전(포인트)·손님 예약금 일반결제는 **토스 그대로**(이건 작동 중).
- **셋업(정우님 콘솔)**: KCP PG-API 인증서/개인키 발급 + 정기자동결제 그룹아이디(`IP7D51056887`, KCP 상점관리자>결제관리>자동결제>그룹관리) → 포트원 admin 채널 "KCP 정기결제"(kcp_v2, 실연동) 등록(인증서·개인키·그룹ID 입력) → Supabase Edge Secret 3개: `PORTONE_STORE_ID`(store-be35728b-…e14b6691b)·`PORTONE_CHANNEL_KEY`·`PORTONE_V2_API_SECRET`.
- **Edge Functions(배포)**: ① `billing-issue` v17 — purpose=subscription이면 포트원 분기: 브라우저 `requestIssueBillingKey`가 빌링키 직접 발급 → 서버는 `GET /billing-keys/{key}`(API secret) 검증·카드정보 → `billings` 저장 + `billing_subscriptions.billing_id` 연결 + 등록 즉시 첫 결제(subscription-charge). ② `billing-charge` v16 — subscription이면 `POST https://api.portone.io/payments/{paymentId}/billing-key`(Authorization: `PortOne {secret}`, body billingKey/orderName/customer.id/amount.total/currency KRW). paymentId=영숫자 40자내. ③ `payment-info` v20 — billing=1 분기에서 포트원 store_id·channel_key 반환(provider:'portone'). **subscription-charge·pg_cron(subscription-billing-daily 02:00 KST batch)은 PG무관이라 무변경.** 손님 정기결제(비-subscription)는 각 함수에 토스 레거시 분기 보존.
...
### v3.8.192 — BlissAI 설정확대 Phase 2: 태그 CRUD + 스탬프/이벤트 토글 (2026-07-01)
Phase 1(알림톡·AI자동응대·제품)에 이어 **태그 관리 + 재방문 스탬프/이벤트 온오프** 액션 5종 추가. 동일하게 미리보기→확인카드→실행, 권한 owner(자기 사업장).
- **`actionSchemas.js`**:
  - `TAG` 신규: **`create/update/delete_tag`**(table:'service_tags', generic op create/update/delete 재사용, fieldsAllowed `name·color·use_yn`, targetField 'name', schedule_yn 미지정→DB 기본 false=예약 태그).
  - `PROGRAM` 신규: **`toggle_stamp_program`**(settings.stamp_program.on) + **`toggle_events_master`**(settings.events_master_enabled — 이벤트 엔진 마스터) + **`toggle_event`**(settings.events[] 이름으로 `enabled` flip, target=이벤트명). 전부 changes `{on}`.
  - `ACTION_SCHEMAS`에 `...TAG, ...PROGRAM` 등록 + buildWriteIntentPrompt에 태그·스탬프·이벤트 예시.
- **`actionRunner.js`**:
  - findTarget sourceMap에 **`service_tags: data?.serviceTags || []`** 추가(update/delete_tag 대상 해결).
  - `_readBizSettings(data)` 헬퍼(data.businesses[0].settings 파싱 — 미리보기 현재값).
  - buildPreview: toggle_stamp_program/toggle_events_master(현재↔변경 on/off) + toggle_event(이벤트명 매칭·현재 enabled·못 찾으면 등록 이벤트 목록 안내).
  - executeAction: 3 토글 전부 **settings fresh 재조회 후 병합**(다른 설정 stale 덮어쓰기 방지) — stamp_program.on / events_master_enabled / events[].enabled(이름 exact→부분 매칭). 태그는 generic CRUD(sb.insert/update raw snake_case, toDb 미적용 → service_tags 컬럼 정합).
...
### 인스타 계정 blissme.ai_official 메시지함 추가 + 전용 SaaS AI (2026-07-01, 서버/DB, React 변경 0)
받은메시지함에 **blissme.ai_official**(브랜드/SaaS 인스타) DM을 추가. 하우스왁싱(biz_khvurgshb) 받은메시지함의 **강남점 아래**에 표시(seoul 계정과 동일 방식).
- **계정**: user_id `17841423954085459`(=account_id, IG-scoped, BUSINESS). Meta 앱 "Bliss Messaging-IG"(IG앱ID 1652568129483097) OAuth "액세스 토큰 생성"으로 장기 토큰(IGAA…, 60일) 발급 + `messages` 웹훅 구독 자동.
- **배선 5**: ① 서버 발송 `BLISS_IG_TOKENS_B64`(secrets.conf, base64 JSON) 9→10개 + daemon-reload·bliss-naver 재시작(running env 확인, 백업 `secrets.conf.bak_igadd_*`) ② Edge Function `app_secrets.ig_tokens` 10개 동기화 ③ 지점 매핑 `businesses.settings.ig_branch_override`에 `17841423954085459→br_4bcauqvrb`(강남) 추가(jsonb_set 경로 지정, 타 설정 보존) ④ 웹훅 구독 `messages` 확인 ⑤ 자동 갱신 = `ig_token_refresh.py`(주간 타이머)가 BLISS_IG_TOKENS_B64 전체 refresh → 새 계정 포함.
- **IG 웹훅 핸들러는 allowlist 없이 generic**(`instagram_webhook`은 entry.id=ig_id 기준 처리) → 새 account_id DM도 그대로 messages INSERT(business_id=BUSINESS_ID, account_id=17841423954085459).
- **blissme.ai 전용 SaaS AI** (`ai_booking.py`, 백업 `ai_booking.py.bak_blissme_*`): `ai_booking_agent` 최상단(미디어 스킵 직후)에 **early-return 분기** — `account_id in BLISSME_PRODUCT_ACCOUNTS`(={17841423954085459})면 `_blissme_product_agent`로 응대하고 왁싱 예약 로직 전부 우회. `BLISSME_PRODUCT_SYSTEM`(제품 소개·요금 Trial 한달/Starter 33k/Pro 77k·무료체험/데모 안내·특정 왁싱 시술 예약 금지) + `_ai_ask_msgs`(gemini-3.5-flash). 채널 on/off·직원 응대중 게이트는 자동응답에만, manual(✨)/suggest_only는 우회. 정우님 결정 "blissme.ai용 AI 별도 설정"(왁싱 컨텍스트로 답하면 곤란).
- **검증**: 토큰 `graph.instagram.com/me`로 user_id/username 확인 + refresh 60일 확인. blissme 전용 AI suggest_only 3케이스 — 한국어 요금문의→SaaS 안내 / 왁싱 예약요청→"프로그램 회사라 시술예약 불가, 해당 왁싱샵 네이버/카톡으로" 정확 거절 / 영어 네일샵 도입문의→영어 SaaS 응대. 전부 정상. **왁싱 골든 게이트 42 PASS + 3 XFAIL + 1 XPASS**(회귀 0 — 내 변경은 blissme account early-return이라 왁싱 케이스 미진입). `price_male_complex`는 문서화된 플레이크(재실행 통과).
- **유의**: ① AI 자동응대는 **채널(instagram) 단위** on/off라, blissme.ai도 그 토글을 공유 — 왁싱 자동응대 컨텍스트와 섞이지 않게 **account_id 분기**로 격리(위). 새 blissme 제품용 계정 추가 시 `BLISSME_PRODUCT_ACCOUNTS`에 account_id 추가 ② ai_booking.py는 서버 전용(git 미추적) — ssh+scp+restart, 앱 버전업·CF퍼지 무관 ③ 앱 inbox는 `ig_branch_override`(settings)를 로드 시 읽음 → 기존 앱 코드 무변경(React 배포 없음), 사용자는 새로고침 1회로 반영 ④ **위례·천호점은 IG 토큰은 있으나 branches.instagram_account_id가 null**(지점 매핑 비어있음) — 별도 정리 대상.

### 골든 게이트 하니스 fix — gender_short_kr 케이스 `ideal` 필드 누락 (2026-07-01)
...
### v3.8.196 — 거래관리 원장 모드 원페이지 UI (탭 제거) + 소프트 스타일 (2026-07-01)
작업세션이 워킹트리에 준비한 `TradesPage.jsx` 변경을 배포세션이 빌드·배포. 거래관리(도매 구매주문) 화면을 **원장(지점) 모드 = 원페이지**로 단순화.
- **관리자 모드에서만 탭 노출**(주문접수·거래내역·거래처·제품·공급자·설정). **원장 모드는 탭 없이 원페이지** — `<OrdersTab onePage>`로 거래내역 목록 + 상단 "주문" 버튼(접힘/펼침) 하나. 원장이 탭 헤맬 필요 없이 바로 구매신청.
- `AdminTrades`: `!adminMode`면 OrdersTab(onePage) 단독 렌더, `adminMode`면 기존 탭 UI. OrdersTab에 `onePage` prop(formOpen 기본 = 원장 접힘/관리자 펼침).
...
### v3.8.200 — 사이드바 카테고리 라벨 숨김 (예약 관리·고객 관리·매출 관리·거래 관리·시스템) (2026-07-02)
정우님 요청 — 사이드바 좌패널의 카테고리 그룹 헤더 라벨 5개(예약 관리·고객 관리·매출 관리·거래 관리·시스템·기타)를 숨겨서 **항목만 나열**되게 함. 그룹 구조는 유지(같은 그룹은 여전히 연달아 표시).
- `Sidebar.jsx` cats.map 내부 `<div>{cat.label}</div>` 렌더링을 `{false && <div>...</div>}`로 감쌈. **코드는 보존**(복구는 `false→true` 한 줄). cats 배열 구조·필터·항목 렌더링·순서 무변경.
- **결과** — 항목만 세로 나열: 타임라인 → 예약목록 → 고객관리 → 마케팅 → 매출관리 → (거래관리) → 사용자관리 → 받은메시지함 → 블리스AI → 관리설정 → 세팅마법사 → 공지&요청 → 직원 후기.
- **검증**(프리뷰 demo owner): 카테고리 라벨 5개(예약 관리·고객 관리·매출 관리·거래 관리·시스템) 등장횟수 각 0회 확인 + 스크린샷으로 라벨 완전 사라짐·항목 정상 노출·순서 유지 확인. 콘솔 에러 0.
- 적용: v3.8.200 라이브 배포(version.txt·CF퍼지).
- **유의**: 그룹 구조 자체는 살아있어 나중에 라벨을 다시 켜거나 다른 표시(구분선 등)로 바꾸기 쉬움. MobileBottomNav("더보기") 항목은 별개 로직이라 무영향.

### v3.8.201 — 사이드바 버전 표시 줄 + 팀채팅 확성기(공지 전송) 버튼 숨김 (2026-07-02)
정우님 스크린샷 요청 — 사이드바 좌하단 두 곳(빨간 표시) 삭제:
- **① 사이드바 버전 표시 줄** (`Sidebar.jsx` 113~124): `v{BLISS_V} · 서버 v{serverV} · [스크래퍼 상태]` 한 줄을 `{false && ...}`로 숨김. (앱 버전은 로그인 화면 하단 `앱 v3.8.201` 표기·요금제·잔액 카드는 그대로 유지.)
- **② 팀채팅 확성기 버튼** (`ChatInput.jsx` 52~73): 팀채팅 입력창 좌측 `공지로 전송 (전체 화면 배너)` 토글 버튼을 `{false && ...}`로 숨김. `announce` state·textarea·전송 로직은 유지(복구 즉시 가능).
- **코드는 보존**(복구는 각 파일 `false→true` 한 줄).
- **검증**(프리뷰 demo owner + 스크린샷): 사이드바 하단 버전 줄 완전 사라짐 + 팀채팅 하단 확성기 아이콘 완전 사라짐 + 항목·팀채팅 텍스트 입력·전송·펼치기 등 나머지 기능 정상. 콘솔 에러 0.
- 적용: v3.8.201 라이브 배포(version.txt·CF퍼지).
- **유의**: 팀채팅 확성기 버튼은 원래 마스터가 채팅을 **공지 배너**로 승격시키는 기능이었음(v3.3.55~60). 필요 시 `ChatInput.jsx`의 `{false && (<button ...>)}`을 `true`로만 바꾸면 즉시 복구. 사이드바 버전 줄도 동일.

### v3.8.202 — 관리설정 맨 아래 이동 + 직원 후기 사이드바 제거 (2026-07-02)
정우님 요청 — 사이드바 nav 재정렬:
- **관리설정 위치**: 받은메시지함 뒤 → **공지&요청 뒤 맨 아래**로 이동.
- **직원 후기 항목 삭제** (staff·일반 두 nav 모두). `AppShell.jsx`에서 `{ id:"review", ... href:"/review.html" }` 라인을 주석 처리(복구 가능).
- 라우트/PAGE_ROUTES는 별도 처리 없음 — nav에서만 빠지므로 URL 직접 접근은 그대로 가능(외부 review.html 링크 기능 자체는 살아있음).
- **결과 사이드바 순서** (owner/manager, `Sidebar.jsx` cats 그룹 순서 반영): 타임라인 → 예약목록 → 고객관리 → 마케팅 → 매출관리 → (거래관리) → 사용자관리 → 받은메시지함 → 공지 & 요청 → **관리설정**(맨 아래). 직원 후기 없음.
